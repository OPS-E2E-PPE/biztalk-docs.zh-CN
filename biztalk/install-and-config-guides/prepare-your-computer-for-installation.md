...
- **Windows 8.1、 Windows Server 2012 和 Windows Server 2012 R2**： 单击键盘和类型上的 Windows 按钮**Windows Update**。 在搜索结果中，单击“Windows 更新”。  
...
- **Windows Identity Foundation 3.5**： 使用 CSOM 安装选项时，SharePoint 适配器需要 Windows Identity Foundation (WIF)。 [附录 b： 安装 Microsoft SharePoint 适配器](../install-and-config-guides/appendix-b-install-the-microsoft-sharepoint-adapter.md)介绍 SharePoint 适配器的 CSOM 安装选项。    
...
-   Microsoft Office Excel 需要由业务活动监视 (BAM) 在 BizTalk Server 中。 可以使用 BAM Office Excel 工作簿来定义要监视的业务流程。 还可以使用 BAM Excel 工作簿定义业务用户查看 BAM 所收集的数据的方式。  
...
-   数据库邮件使用 SMTP 服务器发送 BAM 警报。 在 BizTalk Server 上或在另一台 IIS 服务器上，可以本地安装 SMTP 服务器。 [附录 D：创建 SMTP 服务器](../install-and-config-guides/appendix-d-create-the-smtp-server.md)中列出了安装和配置 SMTP 服务器的步骤。  
...
2. 下载并安装以下组件的相应平台包：  
   ...
   - 超链接"<http://download.microsoft.com/download/3/1/6/316FADB2-E703-4351-8E9C-E0B36D9D697E/sqlncli_x64.msi>"X64 程序包 (sqlncli_x64.msi)  
   ...
   - 超链接"<http://download.microsoft.com/download/3/1/6/316FADB2-E703-4351-8E9C-E0B36D9D697E/SQLServer2005_NS.msi>"X86 程序包 (SQLServer2005_NS.msi)  
  
   - 超链接"<http://download.microsoft.com/download/3/1/6/316FADB2-E703-4351-8E9C-E0B36D9D697E/SQLServer2005_NS_x64.msi>"X64 程序包 (SQLServer2005_NS_x64.msi)  
...
1. 打开“事件查看器”：  
    ...
    **Windows Server 2012** ： 单击键盘和类型上的 Windows 按钮**事件查看器**。 在“结果”窗口中，单击“事件查看器”。  
