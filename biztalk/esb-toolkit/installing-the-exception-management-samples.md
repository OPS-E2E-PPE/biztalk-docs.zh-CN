---
title: "安装异常管理示例 |Microsoft 文档"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: 46431a8a-29d1-488d-9b16-fedaba2300d9
caps.latest.revision: "5"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 9687bfe561a3b9196ecae1f34968c8d1a1b3e8cf
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/20/2017
---
# <a name="installing-the-exception-management-samples"></a>安装异常管理示例
本部分介绍将 ESB 异常管理示例安装到 Microsoft BizTalk Server GlobalBank.ESB 应用程序的过程。 你可以安装使用附带的安装脚本 ESB 异常管理示例[!INCLUDE[esbToolkit](../includes/esbtoolkit-md.md)]。 本节包含下列主题：  
  
-   [安装使用安装脚本异常管理示例](../esb-toolkit/installing-the-exception-management-samples-using-install-scripts.md)  
  
-   [程序集和项目安装异常管理示例](../esb-toolkit/assemblies-and-artifacts-installed-by-the-exception-management-samples.md)  
  
> [!NOTE]
>  [!INCLUDE[esbToolkit](../includes/esbtoolkit-md.md)]示例应用程序驻留在名为 GlobalBank.ESB BizTalk 应用程序。 若要确保此应用程序存在，请首先安装动态解析示例。  
  
 有关如何运行这些示例以及它们如何工作的信息，请参阅以下主题：  
  
-   [运行修复并重新提交自定义异常处理程序示例](../esb-toolkit/running-the-repair-and-resubmit-custom-exception-handler-sample.md)  
  
-   [运行保留自定义异常处理程序的示例消息](../esb-toolkit/running-the-message-persisting-custom-exception-handler-sample.md)  
  
-   [运行 BizTalk 失败消息路由 ESB 处理示例](../esb-toolkit/running-the-biztalk-failed-message-routing-esb-processing-sample.md)