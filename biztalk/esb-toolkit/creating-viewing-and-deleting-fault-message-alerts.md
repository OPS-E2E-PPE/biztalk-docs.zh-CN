...
4.  在**条件**部分的添加警报页，选择一个字段 (如**应用程序**，**错误类型**，或**故障严重性**) 中**条件**下拉列表; 选择一种比较类型 (如 +、 =、 ！ =、 > =、 \<=、 或**如**) 中**运算符**下拉列表; 和类型或从第三个的列表或文本框中选择的值 (名为**值**)。 当你选择**条件**值，页面将发生变化以显示一个文本框或匹配的值的下拉列表。 所有条件将都结合使用**AND**运算符。  
