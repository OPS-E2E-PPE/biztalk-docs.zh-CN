---
title: "开发活动 |Microsoft 文档"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: 1617fa8c-6560-4c1e-aa8a-8ce558a8db04
caps.latest.revision: "2"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: b574d7f49904088f9a4449ceea78ad3ec6ed8eaf
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/20/2017
---
# <a name="development-activities"></a>开发活动
本部分介绍开发的活动[!INCLUDE[esbToolkit](../includes/esbtoolkit-md.md)]方案使用路线设计器，包含以下主题：  
  
-   [开发活动的先决条件](../esb-toolkit/prerequisites-for-the-development-activities.md)  
  
-   [如何： 动态路由基于消息上下文使用业务规则策略在一条消息](../esb-toolkit/dynamically-route-messages-based-on-message-context-using-business-rules-policy.md)  
  
-   [如何： 将一条消息路由到多个收件人使用路线路由的滑动](../esb-toolkit/route-a-single-message-to-multiple-recipients-using-an-itinerary-routing-slip.md)  
  
-   [如何： 转换一条消息，并将生成的消息路由到使用路线的路由滑动某个文件位置](../esb-toolkit/transform-message-and-route-the-message-to-a-location-using-itinerary-routing.md)  
  
-   [如何： 转换消息和使用请求-响应消息交换模式的服务终结点的路由](../esb-toolkit/transform-message-and-route-to-service-endpoint-using-request-response-message.md)  
  
-   [如何： 选择使用业务规则策略路线](../esb-toolkit/how-to-select-an-itinerary-using-a-business-rules-policy.md)  
  
-   [如何： 拆分将交换，并将生成的消息路由到多个使用不同路线的文件位置](../esb-toolkit/split-an-interchange-and-route-messages-to-multiple-locations-using-itineraries.md)  
  
-   [如何： 将文本文档转换为 XML 和路由到使用路线的路由滑动的文件位置](../esb-toolkit/convert-text-document-to-xml-and-route-to-location-using-itinerary-routing-slip.md)  
  
-   [如何： 创建一条路线动态将一条消息路由到使用的 LDAP 查询的电子邮件地址](../esb-toolkit/create-itinerary-to-dynamically-route-message-to-an-email-address-using-ldap.md)  
  
-   [如何： 实现基于内容的路由使用业务规则的一个已知的消息类型的策略](../esb-toolkit/apply-content-based-routing-using-business-rules-policy-for-known-message-type.md)  
  
-   [如何： 验证使用 ESB 入口的消息](../esb-toolkit/how-to-validate-a-message-using-an-esb-on-ramp.md)  
  
-   [如何： 将服务发布到 UDDI 3.0 注册表](../esb-toolkit/how-to-publish-a-service-to-the-uddi-3-0-registry.md)  
  
-   [如何： 解决使用 UDDI 类别搜索的服务终结点](../esb-toolkit/how-to-resolve-a-service-endpoint-using-a-uddi-category-search.md)  
  
-   [如何： 解决使用绑定密钥搜索 UDDI 服务终结点](../esb-toolkit/how-to-resolve-a-service-endpoint-using-a-uddi-binding-key-search.md)  
  
-   [如何： 启用 BAM 在 ESB 路线服务中跟踪](../esb-toolkit/how-to-enable-bam-tracking-in-an-esb-itinerary-service.md)  
  
-   [如何： 实现基于内容的路由使用消息上下文属性](../esb-toolkit/how-to-implement-content-based-routing-using-message-context-properties.md)