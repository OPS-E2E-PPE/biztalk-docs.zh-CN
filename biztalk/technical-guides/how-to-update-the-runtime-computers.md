...
1.  将编辑的 SampleUpdateInfo.xml 文件复制到 files\microsoft [!INCLUDE[prague](../includes/prague-md.md)]\Schema\Restore 目录在每个 32 位 BizTalk 服务器上或者为 \Program Files (x86) \Microsoft[!INCLUDE[prague](../includes/prague-md.md)]上每个 64 位的 \Bins32\Schema\Restore 目录在目标系统中的 BizTalk server。  
...
3.  在命令提示符处，导航到 files\microsoft [!INCLUDE[prague](../includes/prague-md.md)]（在 32 位计算机） 上的 \Schema\Restore 或 \Program Files (x86) \Microsoft [!INCLUDE[prague](../includes/prague-md.md)]\Bins32\Schema\Restore （在 64 位计算机） 上的，键入此命令：  
