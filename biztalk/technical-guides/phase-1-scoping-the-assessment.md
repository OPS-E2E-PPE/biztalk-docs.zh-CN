...
### <a name="message-load"></a>消息负载  
 请务必考虑直接从一开始就你要复制的消息负载，实际上将经过生产系统的方式。 例如，如果将在生产环境中的消息的 20%< 大小的 20 KB，50%将\<在大小和剩余 30%的 100 KB 可能是最大为 1 MB 大小，在很重要，这在实验室中复制。  
...
-   **测试主管**-确保获得准确、 完整和全面的结果集至关重要。 测试领导负责确保捕获、 适当地分析以及相应地分发每个测试运行完成后所需的信息。 务必要考虑如何捕获数据，通常情况下测试数据是适用于使用 Excel 电子表格的演示文稿。 以下列表说明了应捕获每个测试的运行在本实验中的数据：  
    ...
    |------------|----------------|---------------------|-------------------|---------------------|------------------------------------|--------------------|--------------------------|--------------|  
    |向外扩展|140 消息/秒|0.777 秒|99.3%|2|6|270,000|609 字节|30 分钟|  
