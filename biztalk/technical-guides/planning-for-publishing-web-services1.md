...
[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]为 Web 服务提供内置支持。 它使你重复使用并聚合中你的业务流程将现有的 Web 服务。  
 ...
 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]实现支持通过 SOAP 适配器使用的 Web 服务中的本机适配器。 本机适配器支持为 Web Services 提供了可伸缩性、容错功能和跟踪功能，且无需为此编写代码。 有关 SOAP 适配器的详细信息，请参阅[SOAP 适配器](http://go.microsoft.com/fwlink/?LinkId=155754)(http://go.microsoft.com/fwlink/?LinkId=155754)。  
  
 [!INCLUDE[btsBizTalkServer2006r3](../includes/btsbiztalkserver2006r3-md.md)]提供作为 WCF 服务与 Windows Azure AppFabric Service Bus 终结点发布的 BizTalk 应用程序的支持。 有关详细信息请参阅[使用服务连接 AppFabric](../technical-guides/planning-for-publishing-web-services1.md#BKMK_AFCS)。  
...
### <a name="internet-information-services-70"></a>Internet Information Services 7.0  
 可以将 Web 服务发布到 Windows 系统的 IIS 7.0 或更高版本配置与 ASP.NET 2.0。 IIS 7.0 的所有 Web 服务在 ASP.NET 工作进程中都运行。  
  
 IIS 7.0 使用用于处理 Web 服务请求的 IIS 应用程序池。 IIS 7.0 支持多个应用程序池，每个应用程序池进程可以在不同用户上下文下运行。  
...
### <a name="biztalk-isolated-hosts"></a>BizTalk 隔离主机  
 ...
 BizTalk Server 配置管理器创建的 BizTalkServerIsolatedHost[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]使用作为默认的独立主机。 默认情况下，BizTalk Isolated Host Users 组是与此主机关联的 Windows 组的名称。 有关主机和主机实例的详细信息，请参阅[管理 BizTalk 主机和主机实例](http://go.microsoft.com/fwlink/?LinkID=154191)(http://go.microsoft.com/fwlink/?LinkID=154191)。  
...
-   必须将应用程序池的用户帐户添加到独立主机的相应本地或域组中。 有关详细信息，请参阅[Windows 组和 BizTalk Server 中的用户帐户](http://go.microsoft.com/fwlink/?LinkId=155755)(http://go.microsoft.com/fwlink/?LinkId=155755)。  
  
-   你需要根据上述表格在独立主机实例和相应的应用程序池之间匹配用户帐户。 有关独立的主机实例和应用程序池的用户帐户之间的关系的详细信息，请参阅[如何更改服务帐户和密码](http://go.microsoft.com/fwlink/?LinkId=155756)(http://go.microsoft.com/fwlink/?LinkId=155756)。  
  
-   IIS 5.0 和 Windows 2000 Server 或 Windows XP 中的 IIS 5.1 没有应用程序池。 如果你使用的 IIS 5.0 和 5.1，设置到的 IIS 虚拟目录的隔离级别**高**。 这将为每个虚拟目录创建单独的 COM+ 应用程序。 每个 COM+ 应用程序都将在其自己的 dllhost.exe 中运行。  
...
### <a name="enabling-aspnet-40-for-published-web-services"></a>为发布的 Web 服务中启用 ASP.NET 4.0  
 若要为已发布的 Web 服务中启用 ASP.NET 4.0，请参阅[如何发布 Web 服务中启用 ASP.NET 4.0](http://go.microsoft.com/fwlink/?LinkId=155758) (http://go.microsoft.com/fwlink/?LinkId=155758)。  
  
## <a name="using-the-biztalk-web-services-publishing-wizard"></a>使用发布向导的 BizTalk Web 服务  
 有关发布作为 Web 服务业务流程的详细信息，请参阅[发布作为 Web 服务业务流程](http://go.microsoft.com/fwlink/?LinkId=155761)(http://go.microsoft.com/fwlink/?LinkId=155761)。  
  
 作为 Web 服务发布的架构的详细信息，请参阅[作为 Web 服务的发布架构](http://go.microsoft.com/fwlink/?LinkId=155762)(http://go.microsoft.com/fwlink/?LinkId=155762)。  
  
##  <a name="BKMK_AFCS"></a>用 AppFabric 连接服务  
 终结点有关 BizTalk 应用程序发布为与 Windows Azure AppFabric Service Bus 的 WCF 服务的详细信息请参阅[上使用适用于服务的 AppFabric 连接云中公开 BizTalk 应用程序](http://go.microsoft.com/fwlink/?LinkID=204700)(http://go.microsoft.com/fwlink/？LinkID = 204700)。  
...
## <a name="planning-for-publishing-wcf-services"></a>规划发布 WCF 服务  
 ...
 有关规划中的 WCF 服务的详细信息[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]，请参阅[发布 WCF 服务](http://go.microsoft.com/fwlink/?LinkId=155764)(http://go.microsoft.com/fwlink/?LinkId=155764)。  
