...
> [!NOTE]
>  返回的结果集执行对服务实例的查询时，将显示的值**\<名称不可用 >**为**ServiceName**字段的服务实例如果对应发送端口，接收位置，或业务流程已被删除。  **ServiceName**查找到发送端口的友好名称的 BizTalk 管理数据库填充服务实例的字段、 接收位置或业务流程。  如果发送端口接收位置，或删除业务流程则友好名称查找失败和**\<名称不可用 >**显示。  
