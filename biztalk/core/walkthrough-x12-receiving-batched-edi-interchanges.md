...
---
# <a name="walkthrough-x12-receiving-batched-edi-interchanges"></a>演练 (X12):接收批处理的 EDI 交换
本演练提供了一的分步过程，将创建一个解决方案以接收 EDI 批使用[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]。 此解决方案演示了两种接收批处理的 EDI 交换的方法：  
...
## <a name="how-the-solution-splits-received-edi-batches"></a>解决方案如何拆分接收的 EDI 批  
 时配置解决方案以将批处理的交换拆分为其组成事务集，解决方案将执行以下操作：  

1. 接收位置接收批处理的 EDI 交换，它包含从参与方的多个事务集。  

   > [!NOTE]
   >  显示的顺序可能不是此列表中的事件。  

2. 接收管道将拆分为事务集转换为内部 XML 格式其组成事务集收到的交换。  

3. 接收管道将整个交换和组标头的每个事务的上下文从交换集拆分。 它还会升级某些特定交换和组标头，如 ISA6、 GS1 和 GS2，以便这些字段可用于路由。  

4. 接收管道将每个事务集 XML 文件放入 MessageBox 中。  

   > [!NOTE]
   >  在此解决方案中，批处理包含相同的消息类型的多个实例。  

5. 发送端口提取每个事务集通过订阅适当的上下文属性。  

6. 发送管道生成每个事务集成 EDI 交换，并随后将交换发送到目标。  
   ...
   下图显示了体系结构和批处理交换中的消息流的解决方案是配置为拆分事务集。  
...
## <a name="how-the-solution-preserves-received-edi-batches"></a>解决方案如何保留接收的 EDI 批  
 在配置解决方案以保留批处理的交换时，解决方案将执行以下操作：  

1. 接收位置接收批处理的 EDI 交换包含来自参与方的多个事务集。  

2. 接收管道处理交换而不拆分事务集，并将两个事务集作为一个单元为内部 XML 格式。  

3. 如果如交换不是批处理中，使用它将保留的标记应用到的 XML，它将生成的异常，则接收管道将升级相同的属性。 此标记\<X12InterchangeXml\>对于 X12 编码的 EDI 交换或\<EdifactInterchangeXml\>对于 EDIFACT 编码的 EDI 交换。 EDI 接收管道还应用上下文属性`ReuseEnvelope`以将交换标识为保留。  

   > [!NOTE]
   >  EDI 发送管道使用\<X12InterchangeXml\>或\<EdifactInterchangeXml\>标记来标识为保留批的消息。 `ReuseEnvelope`上下文属性，可创建用于订阅保留的所有批处理交换的发送端口。  

4. 接收管道将消息 XML 文件放入 MessageBox 中。  

5. 发送端口通过订阅适当的上下文属性来提取交换。  

6. 发送管道生成的两个事务批处理的 EDI 交换，在 XML 文件中设置，然后将交换发送到目标。  
...
-   出于测试目的，该解决方案到本地文件夹使用发送端口以发送拆分的交换或保留的批。  
...
- 创建接收端口的[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]以接收 EDI X12 编码的.txt 批处理输入的消息从参与方。  

- 创建发送端口的[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]生成每个事务交换设置收到批处理 （如果拆分批） 中或以生成单个交换中接收的批中的事务集 （如果保留批）。 然后，发送端口将交换发送，或到目标参与方的交换。 此发送端口将是一个静态单向发送端口。  

- 为参与方 A 和参与方 B 创建参与方 （贸易合作伙伴）  

- 为这两个贸易合作伙伴创建业务配置文件每个。  

- 创建通过配置要接收的消息通过拆分或保留交换的 EDI 属性的两个配置文件之间的协议。  

- 一个批处理的 EDI 交换放入与接收位置关联的本地文件夹。 然后，您可以验证的[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]是否已从接收的批 （如果拆分批） 或单个保留的批处理的交换 （如果保留批） 放入与发送端口关联的文件夹将单独的交换。  

  > [!NOTE]
  >  如果您已执行中的步骤发送批处理交换演练中，您可以使用该解决方案的输出作为输入此解决方案。 该解决方案的输出是两个 850 消息批。 有关详细信息，请参阅[演练 (X12):发送批处理 EDI 交换](../core/walkthrough-x12-sending-batched-edi-interchanges.md)。  
...
2. 右键单击你的项目，指向**外**，然后单击**现有项**。 将移动到**\<驱动器\>: \Program Files\Microsoft BizTalk Server 2009\XSD_Schema\EDI\X12\00401**，然后双击与测试消息对应的架构。  
   ...
   > 
   > [!NOTE]
   >  对于测试消息，可以使用由发送批处理的交换演练中的解决方案生成的批处理的交换。 该解决方案的输出是用于 EDI 接口开发人员教程的 850 示例消息的两个实例一个批。 如果这样做，则必须使用 x12_00401_850.xsd 架构位于[!INCLUDE[btsBiztalkServerPath](../includes/btsbiztalkserverpath-md.md)]SDKEDI 接口开发人员 TutorialInbound_EDI。  

3. 设置程序集密钥文件，然后生成并部署该程序集。  

##### <a name="to-create-a-one-way-receive-port-to-receive-the-batched-edi-messages"></a>若要创建一个单向接收端口以接收批处理的 EDI 消息  
...
7. 在执行以下任务**PartyA-> 参与**选项卡。  
   ...
   4. 上**本地主机设置**页**交换设置**部分的**入站消息处理选项**框中，选择**拆分交换为事务集-出错时挂起事务集**选项。  
      ...
      > [!NOTE]
      >  开始时，在此解决方案中，我们会将交换拆分通过选择此选项。 更高版本，作为的一部分[测试演练](../core/walkthrough-x12-receiving-batched-edi-interchanges.md#BKMK_Proc)下面的过程，我们将配置解决方案以保留该交换。  
...
1. 在 Windows 资源管理器，打开与接收位置关联的本地文件夹并测试批处理 EDI 交换放入该文件夹。  

   > [!NOTE]
   >  对于测试消息，可以使用由发送批处理的交换演练中的解决方案生成的批处理的交换输出。 此示例消息将包含两个事务集。  

2. 打开与上面创建的发送端口关联的文件夹。 验证该文件夹包含两个新文件，以及每个文件包含一个事务的测试批处理消息中设置的 850 交换。  
...
6. 在 Windows 资源管理器，打开与接收位置关联的本地文件夹，再一次相同的测试批处理 EDI 交换放入该文件夹。  

7. 在 Windows 资源管理器中，打开与上面创建的发送端口关联的文件夹。 验证该文件夹现在包含一个新文件，并且该文件是包含测试批处理消息中的两个 850 事务集的交换。  
