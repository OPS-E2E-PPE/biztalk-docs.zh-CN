...
**\<等效\>** 节点及其子级中使用架构树以显示出现的复杂数据类型多形性。 当从另一种复杂的数据类型派生一个复杂数据类型时，XSD 内的多形性允许下列任意一种数据出现在实例消息中为其指定基本复杂数据类型的位置。 在架构验证期间在特定位置允许多个可能的复杂数据类型之一这一事实是隐式地表示与关联的基本复杂数据类型名**基**属性**扩展**或**限制**派生的复杂数据类型的元素。 若要使此多形性在架构树中，更显而易见**\<等效\>** 显式显示节点和子节点。  
...
## <a name="xsd-representation"></a>XSD 表示形式  
 没有任何直接 XSD 表示形式**\<等效\>** 节点及其子级。 此节点在架构树中使用可使复杂数据类型多形性更为直观和明显。  
