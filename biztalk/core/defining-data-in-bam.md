...
|业务数据-Float|一个十进制值。 例如，PO 的美元总金额。|  
...
## <a name="progress-dimensions"></a>进度维度  
 BAM 引入了新类型的维度： 进度维度。 通过创建进度维度，可以创建与正在进行的活动的进度相关的聚合。  
...
||拒绝|150|50|  
 ...
 进度维度提供的基于实时聚合 (RTA) 的图表非常有用的信息。 使用 RTA 您可以看到业务流程的当前状态，并能够轻松地确定流程瓶颈。  
  
 进度维度可以是按顺序进行排序中购买的里程碑： 第一步完成后，才能开始下一步。 或者，里程碑可以按关联顺序一前一后地完成。 后续步骤是子步骤，关联步骤是同级步骤。 在采购订单流程中，一接收到采购订单，验证就开始了。 由于 verification 里程碑是与 receive 里程碑同时发生的短暂步骤，因此它是 receive 里程碑的同级。 采购订单只有在收到之后才能进行审批，因此 approved 里程碑是 received 里程碑的子级。  
