...
|--------------|----------|--------------|---------------|  
|CurrentSSOServer|VT_BSTR|信息要发送到的服务器的名称|All|  
...
- **事务**： 指定的 DTC 事务的作用域执行的操作由 SSO 对象模型。 您必须在 `ptrValue` 中传递一个 DTC ITransaction 指针，或者传递“null”以清除当前事务作用域。  
...
- **AsyncCall**： 如果为 true，则 SSO 将执行使用异步远程过程调用 (RPC) 的方法。 该方法在执行过程中会返回 E_PENDING。 任何其他返回值均表明该方法已完成。 AsyncCall 还允许您进行轮询该方法以确定它是否完成。
