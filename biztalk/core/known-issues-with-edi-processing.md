---
title: EDI 处理的已知问题 |Microsoft 文档
ms.custom: ''
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: ''
ms.suite: ''
ms.tgt_pltfrm: ''
ms.topic: article
ms.assetid: b5195457-4873-473f-9f13-6df2483ebe60
caps.latest.revision: 22
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 7a2bff7c8367b5471b5ba58e89cd7fd46c1bcada
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/20/2017
ms.locfileid: "22261949"
---
# <a name="known-issues-with-edi-processing"></a>EDI 处理的已知问题
本部分包含的主题介绍 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] EDI 解决方案的已知问题。  
  
## <a name="in-this-section"></a>本节内容  
  
-   [已知的问题的 EDI 接收处理](../core/known-issues-with-edi-receive-processing.md)  
  
-   [已知的问题的 EDI 发送处理](../core/known-issues-with-edi-send-processing.md)  
  
-   [EDI 确认的已知的问题](../core/known-issues-with-edi-acknowledgments.md)  
  
-   [EDI 批处理的已知的问题](../core/known-issues-with-edi-batching.md)  
  
-   [EDI 方的已知的问题](../core/known-issues-with-edi-parties.md)  
  
-   [EDI 验证、 架构和消息的已知的问题](../core/known-issues-with-edi-validation-schemas-and-messages.md)  
  
-   [使用与 EDI 解决方案一起使用的 XML 工具的已知的问题](../core/known-issues-with-xml-tools-used-with-edi-solutions.md)  
  
## <a name="see-also"></a>另请参阅  
 [EDI 和 AS2 解决方案疑难解答](../core/troubleshooting-edi-and-as2-solutions.md)   
 [EDI 解决方案体系结构](../core/edi-solution-architecture.md)   
 [开发和配置 BizTalk Server EDI 解决方案](../core/developing-and-configuring-biztalk-server-edi-solutions.md)