---
title: 配置 MQSeries 适配器 |Microsoft 文档
ms.custom: ''
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: ''
ms.suite: ''
ms.tgt_pltfrm: ''
ms.topic: article
helpviewer_keywords:
- configuring [MQSeries adapters]
- MQSeries adapters, configuring
ms.assetid: 73290678-a0da-42e6-b86e-389bc71afec3
caps.latest.revision: 18
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 389e044db560cd9c8c8f516f8473289fe9a83156
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/20/2017
ms.locfileid: "22232893"
---
# <a name="configuring-the-mqseries-adapter"></a>配置 MQSeries 适配器
本部分将介绍如何配置 MQSeries 适配器。  
  
 有关的适配器的工作方式的信息，请参阅[MQSeries 适配器组件](../core/components-of-the-mqseries-adapter.md)。  
  
## <a name="in-this-section"></a>本节内容  
  
-   [如何配置 MQSeries 适配器接收位置和发送端口](../core/how-to-configure-mqseries-adapter-receive-locations-and-send-ports.md)  
  
-   [如何配置 MQSeries 适配器发送和接收处理程序](../core/how-to-configure-mqseries-adapter-send-and-receive-handlers.md)  
  
-   [使用 MQSAgent COM + 配置向导](../core/using-the-mqsagent-com-configuration-wizard.md)  
  
-   [无提示 MQSeries 适配器配置](../core/silent-configuration-of-the-mqseries-adapter.md)  
  
-   [MQSeries 适配器属性](../core/mqseries-adapter-properties.md)