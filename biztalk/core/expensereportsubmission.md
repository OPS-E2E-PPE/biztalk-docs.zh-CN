...
|在 \BTSExpenseDemo 文件夹中：<br /><br /> BTSExpenseDemoBinding.xml|用于自动安装，如端口绑定。|  
...
2.  运行 Setup.bat 文件，该文件将执行以下操作：  
    ...
    > [!NOTE]
    >  有关 BizTalk ISAPI 筛选器的详细信息，请参阅[如何将 IIS 配置的 HTTP 接收位置](../core/how-to-configure-iis-for-an-http-receive-location.md)。  
...
3.  setup.bat 文件将本示例的虚拟目录配置为在与默认网站相关联的 IIS 应用程序池中运行。  若要配置此示例中的用户的上下文中运行的虚拟目录**BizTalk 独立主机用户**和**IIS_WPG**用户组，你应配置要运行在新的虚拟目录IIS 应用程序池。  通过完成以下步骤，可将虚拟目录配置为在新的 IIS 应用程序池中运行：  
...
4.  在 IIS 中为 HTTPReceive.dll 添加一个 Web 服务扩展  
    ...
    1.  在**Internet Information Services (IIS) Manager**，导航到**Web 服务扩展**文件夹。  
  
    2.  右键单击**Web 服务扩展**文件夹，然后选择**添加一个新的 Web 服务扩展**以显示**新建 Web 服务扩展**对话框。  
  
    3.  输入**ExpenseReportSubmission**为**扩展名：**  
