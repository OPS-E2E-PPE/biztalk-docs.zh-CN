...
## <a name="using-tracking-profiles-in-the-tpe"></a>在 TPE 中使用跟踪配置文件  
 TPE 的用户可以创建 BAM 活动中的项和这些项的 BizTalk Server 解决方案源之间的映射或跟踪配置文件。 BAM 活动由构成“想看的列表”的里程碑和上下文数据组成，并在业务中定义了跟踪配置文件跟随的工作单位。 有关活动的详细信息，请参阅[与事件流实现 BAM 活动](../core/implementing-bam-activities-with-event-streams.md)。  
