...
## <a name="add-a-custom-wcf-behavior-for-salesforce-authentication"></a>为 Salesforce 身份验证添加自定义 WCF 行为  
 Salesforce 向客户端应用程序提供了不同选项来向 Salesforce 进行身份验证。 我们将使用作为哪些 Salesforce 条款[用户名密码](http://go.microsoft.com/fwlink/?LinkId=287082)流。 在客户端中，WCF 使您能够创建[消息检查器](http://msdn.microsoft.com/library/aa717047\(v=VS.90\).aspx)以更改发送之前或之后接收的消息。 消息检查器是客户端运行时的扩展，已配置为一种行为。 行为进而会添加到行为扩展元素。 此行为扩展元素在 machine.config 中使用元素名称注册，这使它可以在 WCF 端口上配置为一种自定义行为。 有关详细信息，请参阅[的自定义行为扩展 WCF](http://msdn.microsoft.com/magazine/cc163302.aspx)。 我们将会使用此方法整合用于向 Salesforce 进行身份验证的用户名身份验证流程。 我们将执行的广泛步骤如下：  
...
1.  作为的一部分**BtsSalesforceIntegration**解决方案在 Visual Studio 中，创建一个 C# 类库。 将其命名为`Microsoft.BizTalk.Adapter.Behaviors`并添加以下命名空间：  
...
1.  在**BtsSalesforceIntegration**解决方案中，创建一个新[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]项目。 将项目`CustomPipeline`。  
  
2.  在**CustomPipeline**项目中，添加新的接收管道。 命名管道作为`AddNamespace.btp`。  
  
3.  在**解码**管道，从工具箱的阶段拖放**ESB 添加 Namespace**管道组件。 在**拆卸**暂存、 拖动和删除**XML 反汇编程序**管道组件。  
  
    > [!NOTE]
    >  如果**ESB 添加 Namespace**管道组件未列出工具箱中，你可以添加它。 右键单击**BizTalk 管道组件**选项卡上，并依次**选择项**。 在**选择工具箱项**对话框中，单击**BizTalk 管道组件**选项卡上，选中的复选框**ESB 添加 Namespace**组件，然后单击**确定**。  
  
4.  向项目添加强名称密钥文件，并保存项目。  
  
5.  右键单击**CustomPipeline**项目，然后选择**属性**。 在**部署**选项卡上，为**应用程序名称**，输入`SalesforceIntegration`。  
