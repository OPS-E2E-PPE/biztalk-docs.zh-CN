...
当**更新**调用函数对象，该对象会重新添加到引擎会重新评估，基于新数据和状态。 该对象可以是类型**TypedXmlDocument**或.NET 类或**DataConnection**或**TypedDataTable**。  
 ...
 通常情况下，使用**Assert**规则引擎工作内存中放置一个新的对象并使用**更新**更新工作内存中已经存在的对象。 当添加新的对象时，将评估中的所有规则的条件。 更新现有对象，请重新评估使用已更新的事实的条件，并操作添加到议程中，如果这些条件进行计算时为 true。  
  
## <a name="using-update-function-on-net-facts"></a>对.NET 事实使用更新功能  
 需要按照作为示例的两个规则。 假设的对象**ItemA**并**ItemB**工作内存中已存在。 规则 1 评估**Id**上的属性**ItemA**，设置**Id**属性**ItemB**，然后重新添加和**ItemB**在更改后。 当**ItemB**重新添加，它被视为新对象和引擎将重新评估使用对象的所有规则**ItemB**谓词或操作中。 这可确保，是对的新值重新计算规则 2 **ItemB.Id**，按照中的规则 1。 规则 2 可能已失败它已计算，但计算结果为第一次 **，则返回 true**第二次计算。  
...
```  
 ...
 将对象重新添加到工作内存中此功能可在正向链接情况下让用户显式控制行为。 在此示例中，副作用的副作用，但还重新计算规则 1。 因为**ItemA.Id**未更改，规则 1 评估结果仍为**true**并**assert （itemb)** 操作会重新触发。 因此，规则就会创建无限循环情况。  
...
- 在操作 （不在谓词中），才使用实例类型的规则议程中的上的操作将保留在议程中。  
  
- 仅在操作中使用的实例类型的规则不会重新计算。  
  
  因此，任一谓词仅中的实例类型或同时使用谓词和操作的规则将会重新进行评估，并且其操作添加到根据议程。  
...
```  
 ...
 实现此方法将因规则性质而异。 下面是一个简单的机制来解决上述示例中的问题。  
...
## <a name="using-update-function-on-xml-facts"></a>对 XML 事实使用更新功能  
 需要按照作为示例的两个规则。 假设的。 规则 1 评估采购订单消息中的项的总计数和规则 2 设置为"需要批准"状态的总计数是否大于或等于 10。  
...
```  
 ...
 如果将以下采购订单 (PO) 消息传递作为此策略的输入，你发现即使总项计数为 14，状态未设置为"需要批准"。 这是因为，只在开始评估 rule2 时的值**TotalCount**字段为 0，且不评估规则每次更新时可用的总数。 有条件重新计算每次**TotalCount**是更新，需要调用**更新**函数在父节点 (**项**) 的修改后的节点 (**TotalCount**)。 如果更改 Rule1 如下所示，并对其进行测试一次，你应看到状态字段设置为"需要批准"的值。  
