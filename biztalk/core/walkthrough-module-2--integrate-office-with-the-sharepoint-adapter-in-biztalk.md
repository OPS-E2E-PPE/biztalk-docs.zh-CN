...
-   你必须完成以下演练：[演练： 模块 1-发送和接收消息与 Windows SharePoint Services Adapter](../core/walkthrough-module-1--send-and-receive-messages-with-the-sharepoint-adapter.md)  
  
 有关使用 Windows SharePoint Services Adapter 多服务器部署中的信息，请参阅[设置和部署 Windows SharePoint Services Adapter](../core/setting-up-and-deploying-the-windows-sharepoint-services-adapter.md)。  
...
4.  下**模板**，选择**空 BizTalk 服务器项目**。  
  
5.  类型`OrderProcess`中**名称**字段。  
  
6.  键入你的工作目录中的文件路径**位置**字段。 例如， `C:\WSSAdapterWalkthrough\`。  
  
7.  单击 **“确定”**。  
  
#### <a name="associate-the-key-file-with-the-assembly"></a>将密钥文件与程序集相关联  
  
1.  在解决方案资源管理器，右键单击`OrderProcess`项目，，然后单击**属性**以启动项目设计器。  
...
1.  在解决方案资源管理器，右键单击`OrderProcess`项目中，单击**添加**，然后单击**新项**。  
  
2.  下**类别**，单击**架构文件**。  
  
3.  下**模板**，单击**架构**。  
  
4.  类型`OrderProcessSchema`中**名称**字段，然后再单击**添加**。  
  
5.  中的属性窗口`OrderProcessSchema`，选择`Qualified`为**元素 FormDefault**属性。  
  
6.  中的属性窗口`OrderProcessSchema`，类型`http://OrderProcess.PurchaseOrder`中**目标 Namespace**字段。  
  
7.  在**BizTalk 编辑器**，右键单击`Root`，单击**重命名**，然后键入`PurchaseOrder`。  
...
12. 在属性窗口中，设置**数据类型**属性`Amount`到 xs:unsignedInt。  
  
13. 创建另一个子字段元素并将其命名为 `PurchaseOrderDate`。  
  
14. 在属性窗口中，设置**数据类型**属性`PurchaseOrderDate`到 xs: datetime。  
...
4.  在**名称和描述**部分中，键入`InfoPathSolutions`中**名称字段**。  
  
5.  在**导航**部分中，选择**是**在快速启动栏上显示此窗体库。  
  
6.  在**文档模板**部分中，选择`None`为**文档模板**。  
...
1.  单击**启动**，指向**所有程序**，指向**Microsoft Office**，然后单击**Microsoft Office InfoPath 2007**。  
  
2.  在**填写表单**对话框中，选择**设计窗体。**  
  
3.  在**设计窗体**任务窗格中，选择**从 XML 文档或架构新建**。  
  
4.  在**数据源向导**，单击**浏览**并选择你在上一个过程中创建的架构文件。 例如， `C:\WSSAdapterWalkthrough\OrderProcess\OrderProcess\OrderProcessSchema.xsd`。  
  
5.  单击“下一步” ，然后单击“完成” 。  
  
6.  在**数据源**任务窗格中，右键单击**PurchaseOrder**节点，，然后单击**带有控件的节**。 这将在模板上创建表单。  
  
7.  单击**文件**，单击**保存**，然后单击**保存**。  
  
8.  在**另存为**对话框中，键入`PurchaseOrder.xsn`中**文件名**字段，然后再单击**保存**。  
...
12. 键入的路径和文件名你`InfoPathSolutions`文档库，并依次**下一步**。 例如， `http://<server_name>/sites/WSSAdapterWalkthrough/InfoPathSolutions/PurchaseOrder.xsn`。  
...
4.  类型`http://OrderProcess.PurchaseOrder`中**Namespace**字段，然后再单击**保存并关闭**。  
...
2.  展开**Microsoft[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]管理**，展开**BizTalk 组**，展开**应用程序**，展开**BizTalk 应用程序 1**，然后单击**发送端口**节点。  
  
3.  右键单击`SendToDestination`，然后单击**属性**。  
  
4.  下**传输**，单击**配置**。  
  
5.  在**Filename**字段中，键入`PurchaseOrder2-%XPATH=//pons:PurchaseOrder/pons:PurchaseOrderID%.xml`。  
  
6.  在**Namespace 别名**字段中，键入`pons="http://OrderProcess.PurchaseOrder"`。  
  
7.  在**模板文档库**，类型`InfoPathSolutions`。  
  
8.  在**模板 Namespace 列**，类型`Namespace`。  
  
9. 选择`Yes`为**Microsoft Office 集成**属性。  
  
10. 下**Windows SharePoint Services 集成**，类型`Partner Name`中**列 01**字段。  
  
11. 类型`%XPATH=//pons:PurchaseOrder/pons:BillTo%`中**01 列值**字段中，单击**确定**，然后单击**确定**再次以退出**发送端口属性**对话框。  
  
#### <a name="restart-the-send-port"></a>重新启动发送端口  
  
1.  在**BizTalk 管理控制台**，单击**发送端口**节点。  
  
2.  右键单击`SendToDestination`，然后单击**Unenlist**。  
...
10. 类型`PurchaseOrder2.xml`中**文件名**字段，然后再单击**保存**。  
...
## <a name="next-steps"></a>后续步骤  
 现在，你已完成本演练中，执行[演练： 模块 3-从业务流程访问 SharePoint 属性](../core/walkthrough-module-3-accessing-sharepoint-properties-from-an-orchestration.md)演练用于对你已完成本演练中，扩展集成业务流程到项目，并演示如何访问 SharePoint 中的属性。  
