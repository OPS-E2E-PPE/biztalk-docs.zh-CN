...
4.  对于解决方案中的每个项目，重复执行步骤 1、2 和 3。  
    ...
    |属性|值|解释|  
    |--------------|-----------|-----------------|  
    |应用程序名称|\<Name>|用于将此项目中的程序集部署到的 BizTalk 应用程序的名称。 如果该应用程序已存在，则在你部署该项目时这些程序集将添加到该应用程序中。 如果该应用程序不存在，则将创建该应用程序。 如果此字段为空，则这些程序集将部署到当前组中的默认 BizTalk 应用程序中。 包含空格的名称必须括在双引号 (") 中。|  
