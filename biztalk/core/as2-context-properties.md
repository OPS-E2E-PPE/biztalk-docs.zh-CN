...
## <a name="context-properties-in-the-ediintpropertiesxsd-schema"></a>EdiIntProperties.xsd 架构中的上下文属性  
 EDI/INT 全局属性架构中的消息上下文属性是公开的，因此可以在消息路由等操作中使用这些属性。 在 `Microsoft.BizTalk.Edi.BaseArtifacts` 程序集中的 EdiIntProperties.xsd 中定义了这些上下文属性。 这些属性的命名空间是 `http://schemas.microsoft.com/BizTalk/2006/as2-properties`。 如果对它们进行升级，这些消息上下文属性都可用作`EdiIntAS.<Property Name>`中**筛选器**页**发送端口属性**对话框。  
