...
## <a name="configuration-properties"></a>配置属性
 ...
 使用 BizTalk 浏览器对象模型，您可以通过编程方式创建和配置接收位置。 BizTalk 资源管理器对象模型公开了**IReceiveLocation**接收具有的位置配置接口**TransportTypeData**读/写属性。 此属性以 XML 字符串的名称-值对形式接受 WCF-NetNamedPipe 接收位置配置属性包。 若要在 BizTalk 资源管理器对象模型中设置此属性，必须设置**InboundTransportLocation**属性**IReceiveLocation**接口。  
  
 **TransportTypeData**属性**IReceiveLocation**接口不需要设置。 如果未设置该属性，则 WCF-NetNamedPipe 适配器将使用 WCF-NetNamedPipe 接收位置配置的默认值，如下表所示。  
...
|**SecurityMode**|Enum<br /><br /> -   **无**： 禁用安全性。<br />-   **传输**： 使用基础基于传输的安全提供安全性。 可以使用此模式控制保护级别。|指定使用的安全类型。<br /><br /> 默认值：**传输**|  
...
|**InboundNodeEncoding**|Enum<br /><br /> -   **Base64** -Base64 编码。<br />-   **十六进制**-十六进制编码。<br />-   **字符串**编码文本的 utf-8。<br />-   **XML** -WCF 适配器使用由正文路径表达式中所选节点的外部 XML 创建 BizTalk 消息正文**InboundBodyPathExpression**。|指定的编码，WCF NetNamedPipe 接收适配器使用要解码识别由正文路径表达式中指定的节点类型**InboundBodyPathExpression**。 此属性是必需的如果**InboundBodyLocation**属性设置为**UseBodyPath**。<br /><br /> 默认值： **XML**|  
...
## <a name="configure-variables-for-a-wcf-netnamedpipe-receive-location"></a>WCF NetNamedPipe 接收位置的配置变量  
...
4.  在**接收位置属性**对话框中，在**传输**旁边部分**类型**，选择**WCF NetNamedPipe**从下拉列表，然后单击**配置**。  
  
5.  在**WCF NetNamedPipe 传输属性**对话框中，在**常规**选项卡上，配置终结点地址和 WCF NetNamedPipe 的服务标识接收位置。 有关详细信息**常规**选项卡中**WCF NetNamedPipe 传输属性**对话框中，请参阅**WCF NetNamedPipe 传输属性对话框中、 接收、 常规**选项卡[!INCLUDE[ui-guidance-developers-reference](../includes/ui-guidance-developers-reference.md)]。  
  
6.  在**WCF NetNamedPipe 传输属性**对话框中，在**绑定**选项卡上配置的超时和事务的属性。 有关详细信息**绑定**选项卡中**WCF NetNamedPipe 传输属性**对话框中，请参阅**WCF NetNamedPipe 传输属性对话框中，接收，绑定**选项卡[!INCLUDE[ui-guidance-developers-reference](../includes/ui-guidance-developers-reference.md)]。
  
7.  在**WCF NetNamedPipe 传输属性**对话框中，在**安全**选项卡上，定义安全性功能 WCF NetNamedPipe 接收位置。 有关详细信息**安全**选项卡中**WCF NetNamedPipe 传输属性**对话框中，请参阅**WCF NetNamedPipe 传输属性对话框中，接收，安全**选项卡[!INCLUDE[ui-guidance-developers-reference](../includes/ui-guidance-developers-reference.md)]。
  
8.  在**WCF NetNamedPipe 传输属性**对话框中，在**消息**选项卡上，指定 SOAP 数据选择**正文**元素。 有关详细信息**消息**选项卡中**WCF NetNamedPipe 传输属性**对话框中，请参阅**WCF NetNamedPipe 传输属性对话框中，接收消息**选项卡[!INCLUDE[ui-guidance-developers-reference](../includes/ui-guidance-developers-reference.md)]。
  
## <a name="configure-a-wcf-netnamedpipe-receive-location-programmatically"></a>配置 WCF NetNamedPipe 以编程方式接收位置
