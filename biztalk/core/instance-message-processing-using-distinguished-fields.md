...
将属性提升通过**可分辨字段**机制不需要的属性架构创建。 因为所有属性提升，与你使用**升级属性**对话框中，这是可访问通过使用**升级属性**属性**架构**中的节点消息架构，或通过使用**Promote &#124;显示升级**命令**BizTalk**或快捷菜单。  
  
 在**升级属性**对话框框中，确保**可分辨字段**在对话框中的右侧选择选项卡。 然后展开以查找并选择对话框左侧的架构树中节点**Field 元素**节点或**字段特性**你想要提升为可分辨字段的节点，然后单击**添加**。 有关将提升到的属性的分步说明**可分辨字段**使用**升级属性**对话框中，请参阅[将数据复制到作为 Distinguished 消息上下文字段](../core/how-to-copy-data-to-the-message-context-as-distinguished-fields.md)。  
