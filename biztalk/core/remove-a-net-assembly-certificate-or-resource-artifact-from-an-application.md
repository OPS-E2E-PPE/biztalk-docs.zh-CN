...
2.  键入以下命令，替换为适当的值下, 表中所述：  
    ...
    |参数|Description|  
    |---------------|-----------------|  
    |**/ 应用程序名称**|包含要删除的项目的 BizTalk 应用程序的名称。 如果未指定，则使用默认应用程序。 如果名称包含空格，必须将它括在双引号 （"）。|  
    |**/ Luid**|项目的本地唯一标识符 (LUID)。 你可以通过使用获取而定**ListApp**命令时中, 所述[ListApp 命令](../core/listapp-command.md)。|  
