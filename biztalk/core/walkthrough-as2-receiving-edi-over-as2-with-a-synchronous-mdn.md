...
-   如果运行此演练的计算机安装了 64 位版本的 Windows，则您必须确保 BizTalk 主机标记只能为 32 位。 你还必须确保 IIS 已启用 32 位应用程序设置，应用程序池设置为**True**。 有关详细信息，请参阅[教程 3: AS2 教程](../core/tutorial-3-as2-tutorial.md)。  
  
## <a name="how-the-solution-receives-an-edias2-message-and-returns-a-synchronous-mdn"></a>解决方案如何接收 EDI/AS2 消息并返回同步 MDN  
 该解决方案可进行以下操作：  
  
1.  接收 AS2 消息包含 EDI 交换通过 HTTP 从贸易合作伙伴 Fabrikam，并对从 EDIINT/AS2 交换进行解码。  
  
    > [!NOTE]
    >  此列表中的事件可能不会按所示顺序发生。  
  
2.  返回同步的 MDN 到贸易合作伙伴使用请求-响应接收端口。  
  
3.  将交换的 EDI 格式转换为内部的 XML 格式，并将它放置在消息框。  
  
4.  具有 PassThruTransmit 管道的 FILE 发送端口将提取消息 XML 文件。  
  
5.  发送端口将 EDI 交换 XML 文件发送到 Contoso 方在一个文件夹。  
  
 下图显示出此解决方案的结构。  
  
 ![与同步 MDN 的 AS2 接收](../core/media/4ff20070-1c36-42e5-af14-832771d63b88.gif "4ff20070-1c36-42e5-af14-832771d63b88")  
  
## <a name="the-functionality-in-this-solution"></a>此解决方案中的功能  
 本演练的功能具有以下特点：  
  
-   不会生成 EDI 确认。 生成 EDI 确认进行了演示[演练 (X12)： 接收 EDI 交换和发送回确认](../core/walkthrough-x12--receive-edi-interchanges-and-send-back-an-acknowledgement.md)。 中所述通过 AS2 传输发送 EDI 确认[演练 (AS2): 通过使用同步 MDN 的 AS2 发送 EDI](../core/walkthrough-as2-sending-edi-over-as2-with-a-synchronous-mdn.md)。  
...
-   通过使用提供的是 HTTP 发件人实用工具 AS2 教程文件的一部分测试解决方案。 该实用工具通过 AS2 传输方法发送一条包含有 EDI 交换的测试 AS2 消息（X12_00401_864-Sync.edi，也包括在 AS2 教程中）。 您必须对教程中附带的 HTTP 发送程序和测试消息进行修改。 这些更改将在以下相关各节进行介绍。  
...
4.  在**定义管理路径**页上，在**添加新路径**，然后在**路径**文本框中，输入**Contoso**。 下**类型**，单击**排除路径**，然后单击**确定**。  
  
##### <a name="to-create-a-receive-port-to-receive-the-edi-over-as2-message-and-return-an-mdn"></a>创建通过 AS2 消息接收 EDI 并返回 MDN 的接收端口  
  
1.  在[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]管理控制台中，右键单击**接收端口**节点下的**BizTalk 应用程序 1**节点，指向**新建**，然后单击**请求-响应接收端口**。  
...
10. 在**接收位置**窗格中的 BizTalk Server 管理控制台中，右键单击接收位置，然后单击**启用**。  
  
##### <a name="to-create-a-send-port-to-send-the-edi-payload-to-a-local-folder"></a>创建用于将 EDI 负载发送到本地文件夹的发送端口  
  
1.  在 Windows 资源管理器，创建一个名为的本地文件夹**EDI_to_Contoso**发送 EDI 负载。  
...
8.  打开创建发送到 EDI 负载的 Contoso 本地文件夹 (**\EDI_to_Contoso**)。 验证该文件夹中有一个 .XML 文件。 打开该 XML 文件并验证它包含有一个 864 事务集。  
  
9. 在记事本中，打开测试消息 X12_00401_864 Sync.edi 并验证事务集在输出中的消息**\EDI_to_Contoso**对应于事务集 X12_00401_864 Sync.edi 输入中的本地文件夹消息。  
