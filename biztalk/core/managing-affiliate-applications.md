---
title: "管理 Affiliate 应用程序 |Microsoft 文档"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- managing [SSO applications]
- applications [SSO], managing
- SSO, applications
ms.assetid: 9bc6b120-6849-476a-b3af-c18bf2fa0197
caps.latest.revision: "8"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: dc5a355faed995fb6543b1e8f5810eec14584600
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/20/2017
---
# <a name="managing-affiliate-applications"></a>管理关联应用程序
本部分提供了有关如何创建和配置关联应用程序的信息。  
  
## <a name="in-this-section"></a>本节内容  
  
-   [如何创建关联应用程序](../core/how-to-create-an-affiliate-application.md)  
  
-   [如何删除关联应用程序](../core/how-to-delete-an-affiliate-application.md)  
  
-   [如何更新关联应用程序的属性](../core/how-to-update-the-properties-of-an-affiliate-application.md)  
  
-   [如何启用关联应用程序](../core/how-to-enable-an-affiliate-application.md)  
  
-   [如何禁用关联应用程序](../core/how-to-disable-an-affiliate-application.md)  
  
-   [如何列出关联应用程序](../core/how-to-list-affiliate-applications.md)  
  
-   [如何列出关联应用程序的属性](../core/how-to-list-the-properties-of-an-affiliate-application.md)  
  
-   [如何清除应用程序缓存](../core/how-to-clear-the-application-cache.md)  
  
-   [如何设置 SSO 服务器使用客户端实用工具](../core/how-to-set-the-sso-server-using-the-client-utility.md)  
  
-   [如何显示 SSO 服务器使用客户端实用工具](../core/how-to-display-the-sso-server-using-the-client-utility.md)  
  
-   [如何为使用客户端实用工具的关联应用程序设置凭据](../core/how-to-set-credentials-for-the-affiliate-application-using-the-client-utility.md)