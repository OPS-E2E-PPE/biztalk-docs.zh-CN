...
本主题将介绍如何使用 BizTalk Server 管理控制台为发送端口配置备份传输选项。 如果主传输无法正常工作，则您所指定的备份传输将生效。 中所述配置主传输[如何创建发送端口](../core/how-to-create-a-send-port2.md)。  
...
4. 下表中所述配置备份传输属性，然后单击**确定**。  
   ...
   |**重试次数**|指定在消息失败时发送端口重发消息的次数。 默认值为 3；允许的范围为 0 到 1,000。|  
