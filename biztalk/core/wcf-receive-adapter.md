...
#### <a name="extract-the-content-of-the-element-by-using-an-xpath-expression"></a>使用 XPath 表达式提取元素的内容  
 当选中此选项时，根据 XPath 表达式定位的 SOAP Body 元素内部的元素内部内容将被放置到 BizTalk 消息的正文部分中， 并忽略 Body 元素中的其余数据。 此外需要指定节点编码 — 示例中，XML、 Base64、 Hex 或字符串编码。  
  
> [!NOTE]
>  当选择 XML 编码时，将根据 XPath 表达式定位元素的外部内容，并将其放置到正文部分中。  
  
## <a name="handling-web-services-headers"></a>处理 Web Services 标头  
 接收适配器将标准 Web Services 标头的一部分升级为 BizTalk 消息上下文，以便基于这些标头的值进行更方便的访问和路由。 下表列出了接收适配器将保存为消息上下文的属性。 在以下命名空间定义的属性：http://www.w3.org/2005/addressing和http://schemas.microsoft.com/BizTalk/2006/Adapters/WCF-properties。  
