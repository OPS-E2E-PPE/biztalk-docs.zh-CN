...
MIME 示例演示如何发送管道中执行 MIME 编码。  

## <a name="what-this-sample-does"></a>本示例的用途  
 此示例将 MIMEIn 文件夹配置为接收位置。 在一个文件，将示例文件 ImageInput.gif，在此文件夹中，如[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]处理该消息在此文件中使用以下步骤：  

1.  从接收位置文件夹 MIMEIn 检索消息文件。  

2.  在接收管道传递该消息不变。  

3.  在 MessageBox 数据库中，将消息路由至发送管道。  

4.  在发送管道中，执行 MIME 编码并将文件放入发送适配器文件夹 MIMEOut。  
...
|                        ImageInput.GIF                        |                                                                                           示例输入的文件。                                                                                           |
| SampleMimeEncoding.btproj<br /><br /> SampleMimeEncoding.sln |                                                                              本示例的项目和解决方案文件。                                                                               |
|                SampleMimeEncodingBinding.xml                 |                                                                             用于如端口绑定之类的自动化设置。                                                                             |
|                     SendMimePipeline.btp                     |                                 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] 发送带 MIME 编码器组件的管道文件。                                 |
|                          Setup.bat                           |                                                                               用于生成和初始化本示例。                                                                                |

## <a name="building-and-initializing-this-sample"></a>生成并初始化此示例  
 使用以下过程生成并初始化 MIME 示例。  
...
2. 运行文件 Setup.bat，以执行以下操作：  
   ...
   - 在文件夹中创建的输入 (MIMEIn) 和为本示例的输出 (MIMEOut) 文件夹：  
...
> 
> [!NOTE]
>  如果你选择打开并生成本示例中的项目，而无需运行 Setup.bat 文件，必须首先创建强名称密钥对使用.NET Framework 强名称实用工具 (sn.exe)。 使用此密钥对生成程序集进行签名。 若要撤消 Setup.bat 所做的更改，请运行 Cleanup.bat。 必须运行 Setup.bat 前运行 Cleanup.bat 时间。  
...
1.  将文件 ImageInput.gif 的副本放到 MIMEIn 文件夹中。  

2.  查看在 MIMEOut 文件夹中创建的文本文件。 此文本文件的名称根据消息 ID GUID。 此文件包含输入文件 ImageInput.gif 的 MIME 编码的内容。  
