...
3. setup.bat 文件将本示例的虚拟目录配置为在与默认网站相关联的 IIS 应用程序池中运行。  若要配置本示例中的用户上下文中运行的虚拟目录**BizTalk Isolated Host Users**并**IIS_IURS**用户组，应配置要运行在新的虚拟目录IIS 应用程序池。 通过完成以下步骤，可将虚拟目录配置为在新的 IIS 应用程序池中运行：  
   ...
   7.  将本 SDK 示例的虚拟目录配置为在新应用程序池下运行。 **应用程序池**设置位于**虚拟目录**选项卡的虚拟目录属性对话框。 为本示例创建的虚拟目录为 HttpRequestResponseSample。  
...
1.  在 Internet Explorer 中，导航到http://localhost/RequestResponse/。  
