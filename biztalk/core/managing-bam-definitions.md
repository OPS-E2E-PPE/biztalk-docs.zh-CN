---
title: "管理 BAM 定义 |Microsoft 文档"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- managing [BAM definitions]
- definitions [BAM], managing
- definition files [BAM], managing
- managing [BAM], definitions
- managing [BAM definitions], about managing BAM definitions
ms.assetid: 7aba0e40-b8d3-4afc-9e4c-92392f1f6269
caps.latest.revision: "10"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 93cf3d280d0e615442a4141b7fa41f6ee3566490
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/20/2017
---
# <a name="managing-bam-definitions"></a>管理 BAM 定义
BAM 定义是 BAM 基础结构的一部分。 它定义要跟踪和聚合的数据，以及所跟踪数据的业务最终用户视图。 本部分的主题介绍管理 BAM 定义的各个元素（包括活动、视图、项目和警报）的过程。  
  
## <a name="in-this-section"></a>本节内容  
  
-   [管理 BAM 定义文件所需的用户权限](../core/required-user-rights-for-managing-bam-definition-files.md)  
  
-   [如何部署 BAM 定义](../core/how-to-deploy-bam-definitions.md)  
  
-   [如何删除 BAM 定义](../core/how-to-remove-bam-activities.md)  
  
-   [如何列出对 BAM 基础结构的更改](../core/how-to-list-changes-to-the-bam-infrastructure.md)  
  
-   [如何列出 BAM 活动](../core/how-to-list-bam-activities.md)  
  
-   [如何删除 BAM 活动](../core/how-to-remove-bam-activities.md)  
  
-   [如何列出 BAM 视图](../core/how-to-list-bam-views.md)  
  
-   [如何删除 BAM 视图](../core/how-to-remove-bam-views.md)  
  
-   [如何启用警报](../core/how-to-enable-alerts.md)  
  
-   [如何禁用警报](../core/how-to-disable-alerts.md)  
  
-   [如何删除 BAM 警报](../core/how-to-remove-bam-alerts.md)  
  
-   [如何更新 BAM 项目](../core/how-to-update-bam-artifacts.md)  
  
-   [如何删除已部署的项目](../core/how-to-remove-deployed-artifacts.md)  
  
-   [配置 BAM 警报](../core/configuring-bam-alerts.md)  
  
-   [管理 BAM 警报执行](../core/managing-bam-alert-execution.md)  
  
-   [如何更改计算的警报的频率](../core/how-to-change-the-frequency-with-which-alerts-are-evaluated.md)