...
## <a name="test-code-summary"></a>测试代码摘要  
 为单元测试已启用时**FlatFileReceive**项目， **FFReceivePipeline**与相关联的 C# 类**FFReceivePipeline.btp**派生自**Microsoft.BizTalk.TestTools.Pipeline.TestableReceivePipeline**类。 **FFReceivePipelineUnitTest**中的方法**TestProject1**使用**TestPipeline**方法， **FFReceivePipeline**继承若要测试的平面文件接收管道。 在管道处理完消息后，将根据平面文件架构对输出消息进行验证。 参数**TestPipeline**方法如下所示：  
