---
title: "开发 BizTalk Server 应用程序 |Microsoft 文档"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: 99b56f86-d8e4-4f4a-9ce9-9f476ba88ea8
caps.latest.revision: "20"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 0c71782556d896d0697b73b83e2966f304b77a3b
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/20/2017
---
# <a name="developing-biztalk-server-applications"></a>开发 BizTalk Server 应用程序
本部分包含的开发人员负责创建 BizTalk 项目的信息。 项目是使用 BizTalk 项目系统设计环境中，可用于设计、 组织和生成 BizTalk 应用程序的各种元素创建的。 以下部分将详细介绍此过程：  
  
## <a name="in-this-section"></a>本节内容  
  
-   [开发人员工具](../core/developer-tools.md)  
  
-   [使用 BizTalk 消息传送引擎](../core/using-the-biztalk-messaging-engine.md)  
  
-   [创建使用 BizTalk 编辑器的架构](../core/creating-schemas-using-biztalk-editor.md)  
  
-   [创建使用 BizTalk 映射程序图](../core/creating-maps-using-biztalk-mapper.md)  
  
-   [创建管道使用管道设计器](../core/creating-pipelines-using-pipeline-designer.md)  
  
-   [创建使用业务流程设计器的业务流程](../core/creating-orchestrations-using-orchestration-designer.md)  
  
-   [创建和使用业务规则](../core/creating-and-using-business-rules.md)  
  
-   [使用 Web 服务](../core/using-web-services.md)  
  
-   [使用 WCF 服务](../core/using-wcf-services.md)  
  
-   [有关设计 BizTalk 应用程序的国际注意事项](../core/international-considerations-for-designing-biztalk-applications.md)  
  
-   [部署到 BizTalk 应用程序从 Visual Studio BizTalk 程序集](../core/deploying-biztalk-assemblies-from-visual-studio-into-a-biztalk-application.md)  
  
-   [创建单一登录在应用程序](../core/creating-a-single-sign-on-application.md)  
  
-   [查看具有 BizTalk 程序集查看器的程序集](../core/viewing-assemblies-with-the-biztalk-assembly-viewer.md)  
  
-   [实现消息安全性](../core/implementing-message-security.md)  
  
## <a name="see-also"></a>另请参阅  
 [了解 BizTalk 应用程序部署和管理](../core/understanding-biztalk-application-deployment-and-management.md)   
 [BizTalk 应用程序部署和管理清单](../core/biztalk-application-deployment-and-management-checklists.md)   
 [BizTalk 应用程序部署和管理演练](http://msdn.microsoft.com/library/5321f8e0-1e2a-4ac4-a4a2-fc244071bc5b)