...
### <a name="to-configure-ack-processing-and-validation-settings"></a>若要配置确认处理和验证设置  
...
3.  在网格中，可以定义不同的事务集的不同的验证设置。 在中**验证**页上，执行以下操作：  
    ...
    |--------------|----------------|  
    |**默认**|选中复选框，以定义默认验证设置。|  
    |**UNH2.1**|单击空单元格的列中并从下拉列表中选择事务类型。|  
    |**Edi 类型验证**|选择**EDI 类型**以启用对交换接收方的 EDI （数据元素） 验证。 此验证对事务集数据元素，验证数据类型、 长度限制以及空数据元素和尾部分隔符执行 EDI 验证。 有关详细信息，请参阅[EDI 类型 （数据元素） 验证](../core/edi-type-data-element-validation.md)。 **注意：** 即使未选择此属性，如果在架构批注中开启，将执行跨字段/段验证。|  
