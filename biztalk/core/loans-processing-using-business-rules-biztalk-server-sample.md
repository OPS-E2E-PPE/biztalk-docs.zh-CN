...
2. 查看文件夹中创建的.xml 文件**出**。它包含输入的 XML 消息添加到以下四个状态元素的 XML 结构末尾的数据： **IncomeStatus**， **CommitmentsStatus**， **EmploymentStatus**，并**ResidencyStatus**。  
...
5. 更改任何规则或操作将导致不同的值的方式**IncomeStatus**， **CommitmentsStatus**， **EmploymentStatus**，和**ResidencyStatus**元素。 确保也更改的值求反运算部分 (实质上， **Else**子句) 选择要更改任何规则。  
   ...
   |----------------|-------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------|
   |       @shouldalert        |   Income Status Rule    |                                       如果**BasicSalary** + **OtherIncome** > 0<br /><br /> 然后**IncomeStatus** = **有效**                                        |
   ...
   | !1, !2, !3, !4 |     Negation Rules      |    条件的逻辑**不**规则 1 – 4 中所述相应条件。 引发的操作是更改正在设置的字符串。     |
...
11. 查看文件夹中创建的.xml 文件**出**。它包含输入的 XML 消息添加到以下四个状态元素的 XML 结构末尾的数据： **IncomeStatus**， **CommitmentsStatus**， **EmploymentStatus**，并**ResidencyStatus**。 根据此过程步骤 5 中所做更改的性质，添加到这些元素的数据与以前的运行可能不同。  
