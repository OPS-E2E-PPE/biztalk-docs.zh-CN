...
您可以使用 STRIDE 模型来对 BizTalk Server 部署的潜在威胁进行分类。 STRIDE 是首字母缩写词，派生自以下类别： *S*poofing identity *T*篡改数据， *R*epudiation，*我*表示信息泄露*D*表示拒绝服务和特权提升。 本部分列举了对 BizTalk Server 环境的潜在威胁及相应的威胁缓解机制。  
...
- 恶意用户可能会向 BizTalk Server 发送大量的消息，从而阻止 BizTalk 接收有效的消息。 有关此类威胁的缓解措施的详细信息，请参阅[缓解拒绝的服务攻击](../core/mitigating-denial-of-service-attacks.md)。  
