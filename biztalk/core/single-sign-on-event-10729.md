---
title: "单一登录： 事件 10729 |Microsoft 文档"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: d5b31378-7483-47e4-bfd2-1b4b5d561681
caps.latest.revision: "11"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 65e53bcd0bbe4399fa4dc399329aa5c2afa5d0fc
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/20/2017
---
# <a name="single-sign-on-event-10729"></a>单一登录： 事件 10729
## <a name="details"></a>详细信息  
  
|||  
|-|-|  
|产品名称|企业单一登录|  
|产品版本|[!INCLUDE[btsSSOVersion](../includes/btsssoversion-md.md)]|  
|事件 ID|10729|  
|事件源|ENTSSO|  
|组件|N\A|  
|符号名称|SSO_INFO_PROGRESS_RECORD_WRITTEN|  
|消息正文|已成功创建新的进度文件。%r<br /><br /> 适配器: %1 %r<br /><br /> 文件名: %2 %r<br /><br /> 记录数： %3|  
  
## <a name="explanation"></a>解释  
 此信息性事件表明已经成功创建新的进度文件。  
  
## <a name="user-action"></a>用户操作  
  
-   不必进行用户操作。  
  
 有关详细信息，请参阅下列资源：  
  
-   [如何配置密码同步](../core/how-to-configure-password-synchronization.md)  
  
-   [密码同步](../core/password-synchronization2.md)