...
若要测试对 AdapterManagement 项目所做的所有更改，请重新生成该项目。 在成功生成后，运行“添加适配器元数据”向导以确保所有的内部和外部 XSD 文件已添加到 AdapterManagement 项目。 有关使用添加适配器元数据向导的说明，请参阅[如何向 BizTalk 项目添加适配器元数据](../core/how-to-add-adapter-metadata-to-a-biztalk-project.md)。  

 若要测试对配置架构所做的更改，请打开由 XSD 文件生成的每个属性页以确保它们请求和接受正确的数据。 为此，配置发送端口、 接收位置、 发送处理程序，并接收处理程序中的[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]管理控制台<strong>。</strong>  
