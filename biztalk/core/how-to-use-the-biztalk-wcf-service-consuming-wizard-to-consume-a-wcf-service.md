...
5.  如果你选择 **元数据交换 (MEX) 终结点** 选项 **元数据源** 页上， **元数据终结点** 页将出现。 上 **元数据终结点** 页上，指定正在运行的服务，通过 WS 元数据交换或 Http Get 的下载可提供元数据的 URL。 若要从 URL 中获取的元数据文档，请单击 **获取**。 如果正在运行的服务要求具有基本身份验证方案的用户凭据，请单击 **编辑** 以打开 **BizTalk WCF 服务使用向导** 对话框中，你可以在其中提供用户名和密码以访问运行服务时使用。  
