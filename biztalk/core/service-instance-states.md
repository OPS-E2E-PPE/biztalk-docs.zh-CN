...
|State|解释|  
|-----------|-----------------|  
|**启动**|当前在 MessageBox 中的任何服务实例（例如处于“已挂起(可恢复)”状态或“在断点处”状态的服务实例）在 BizTalk 跟踪数据库中都显示为“已启动”。|  
|**已完成**|服务实例处理已成功完成。|  
|**终止**|服务实例已终止。|  
...
|**排队 （正在等待处理）**|当按序送达发送端口正在重试上一条消息时，该消息处于按序送达状态下。|  
|**排队 （以后再传送计划）**|消息正在等待通过已设置服务时段的发送端口进行发送。|  
|**排队 （正在等待重新尝试）**|该消息与由于目标 URI 不可用而正在尝试重新发送该消息的发送端口相关联。|  
|**已挂起**|“已挂起”包括“已挂起(可恢复)”和“已挂起(不可恢复)”实例状态。|  
|**已挂起 （恢复）**|与消息相关联的服务实例已挂起，并且可以恢复。<br /><br /> 恢复消息实例将执行以下操作：<br /><br /> -恢复消息的实例。<br />-将消息发送到发送端口。<br />-发送端口将消息传送至目标;即使处于已启动状态不是发送端口。|  
|**已挂起 （不恢复）**|与此消息关联的服务实例已挂起，但无法恢复。|  
...
|**已挂起 （恢复）**|在调试器中从断点处恢复|恢复|||Terminate|||  
|**已挂起 （不恢复）**|||||Terminate|||  
|**因未用完消息**|||||Terminate|||  
|**挂起挂起**|可以尝试附加，但最终会失败||停止 Windows 服务|请求已处理|只有在冻结实例时，终止才会起作用|||  
|**挂起的终止**|可以尝试附加，但最终会失败||停止 Windows 服务，冻结实例||请求已处理，或实例已冻结|||  
...
|开始状态|运算||||||  
|--------------------|---------------|------|------|------|------|------|  
||**终止**|**挂起**|**恢复**|**在断点中恢复**|**Continue**|**附加**|  
|**在断点处**|Terminated|已挂起|||在职|在断点处|  
|**在断点处 （冻结）**|Terminated|已挂起|||在职|在断点处|  
|**准备要运行**|Terminated|已挂起|||||  
|**已计划**|Terminated|已挂起|||||  
|**在职**|Terminated|已挂起|||||  
|**冻结**|Terminated|已挂起|||||  
|**已挂起 （恢复）**|Terminated||在职|在断点处|||  
|**已挂起 （不恢复）**|Terminated||||||  
|**因未用完消息**|Terminated||||||  
|**挂起挂起**|已终止；只有在冻结实例时才会起作用|||||争用条件|  
|**挂起的终止**|已终止则仅将适用，已冻结实例时|||||争用条件|  
