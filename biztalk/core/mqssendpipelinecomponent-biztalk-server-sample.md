...
## <a name="what-this-sample-does"></a>本示例的用途  
 此示例由两个[!INCLUDE[btsVStudioNoVersion](../includes/btsvstudionoversion-md.md)]项目、 管道组件项目和一个进行的管道项目使用的管道组件。  
...
2.  修改和编译[!INCLUDE[btsVStudioNoVersion](../includes/btsvstudionoversion-md.md)]管道组件的项目。  
...
1.  创建名为的文件夹**temp**如果不存在你 C:\ 驱动器上。  
  
2.  创建一个文件夹**C:\temp**名为目录**Pickup3**。  
  
## <a name="modifying-and-compiling-the-project-for-the-pipeline-component"></a>修改和编译的管道组件的项目  
 此过程修改和编译[!INCLUDE[btsVStudioNoVersion](../includes/btsvstudionoversion-md.md)]管道组件的项目。  
  
#### <a name="to-modify-and-compile-the-project-for-the-pipeline-component"></a>修改和编译管道组件的项目  
  
1.  双击该解决方案文件， **SetMQSeriesHeaderPropertyComponent\SetMQSeriesHeaderPropertyComponent.sln**以打开中的解决方案[!INCLUDE[btsVStudioNoVersion](../includes/btsvstudionoversion-md.md)]。  
  
2.  双击类文件**CSetMQSeriesHeaderPropertyComponent.cs**以打开中的类文件[!INCLUDE[btsVStudioNoVersion](../includes/btsvstudionoversion-md.md)]。  
  
3.  找到该变量**samplesDir**，验证，此变量设置为位置**C:\temp**。  
  
4.  右键单击解决方案资源管理器中的解决方案，然后单击**生成**。 这会将项目编译为 dll 位于**SetMQSeriesHeaderPropertyComponent\SetMQSeriesHeaderPropertyComponent\bin\Debug\\** 目录。  
...
1.  将已编译的程序集复制**SetMQSeriesHeaderPropertyComponent.dll** BizTalk 管道组件文件夹。 BizTalk 管道组件文件夹的默认位置是 [!INCLUDE[btsBiztalkServerPath](../includes/btsbiztalkserverpath-md.md)] 管道组件。  
  
2.  复制 MQHeader 属性文件**SetMQSMQMDHdrProps.xml**到**C:\temp**目录。  
  
## <a name="modifying-the-project-for-the-biztalk-server-pipeline"></a>对于 BizTalk Server 管道修改项目  
...
1.  在[!INCLUDE[btsVStudioNoVersion](../includes/btsvstudionoversion-md.md)]，通过双击解决方案文件中，打开解决方案**SetMQSeriesHeaderPropertyPipeline\SetMQSeriesHeaderPropertyPipeline.sln**。  
...
3.  在**解决方案资源管理器**，右键单击项目，然后单击**属性**以启动 （在中心窗口中） 为项目的项目设计器。  
  
    1.  在项目设计器中，单击**签名**选项卡。  
  
    2.  在右侧窗格中，选择**对程序集签名**选项...  
...
4.  先前创建的管道组件已添加到**预组建**此管道项目的阶段。 如果尚未添加此组件，则需要完成以下步骤来添加它：  
    ...
    1.  在[!INCLUDE[btsVStudioNoVersion](../includes/btsvstudionoversion-md.md)]IDE 中，单击**工具箱**左侧的选项卡。  
  
    2.  右键单击**工具箱**，然后单击**选择项**。  
  
    3.  在**选择工具箱项**对话框中，单击**BizTalk 管道组件**选项卡上，选择**到设置 MQseries 标头属性的自定义组件**组件，并然后单击**确定**。  
  
    4.  拖动**到设置 MQseries 标头属性的自定义组件**组件**预组建**此管道的阶段。  
...
2.  在**单向接收端口属性**对话框中，在**名称**框中，键入"MQReply"，然后单击**确定**。  
  
3.  在左窗格中，单击**接收位置**选项卡上，并依次**新建**。  
  
4.  在**接收位置属性**对话框中，在**名称**字段中，键入"ReceiveFile"。  
  
5.  在**传输类型**框中，选择**文件**。  
  
6.  在**接收处理程序**字段中，选择**BizTalkServerApplication**。  
  
7.  在**接收管道**字段中，选择**Microsoft.BizTalk.DefaultPipelines.PassThruReceive**。  
  
8.  在**接收文件夹**字段中，键入"C:\temp\Pickup3"。  
  
9. 在**文件掩码**字段中，键入"*。\*"。  
  
10. 单击**确定**，然后单击**确定**再次以退出**接收位置属性**对话框。  
  
## <a name="creating-a-mqseries-queue-through-the-mqseries-explorer"></a>创建 MQSeries 队列通过 MQSeries 资源管理器  
...
4.  在**创建本地队列**对话框中，在**队列名称**，类型**SETHEADER**，然后单击**确定**。  
...
8.  在**MQSeries 传输属性**对话框中，在**队列定义**字段中，单击省略号 （…） 按钮。  
  
9. 在**队列定义**对话框中，在**服务器名称**字段中，键入你的计算机名。  
  
10. 在**队列管理器**字段中，选择默认的队列管理器。  
  
11. 在队列字段中，键入"SETHEADER"，然后单击**导出**。  
  
12. 在**导出**对话框中，单击**创建队列**，然后单击**确定**或**完成**之前已退出所有对话框。  
...
3.  在细节窗格中，右键单击**SetMQHeader**发送端口和单击**启动**。  
...
2.  启动 WebSphere MQ Explorer，双击 SETHEADER 队列以检查 SETHEADER 队列中的消息。  
    ...
    2.  在**消息浏览器**对话框中，单击**列**以显示**消息的显示/隐藏列**对话框。  
  
    3.  下**可用列**，双击每个条目，以使其可见中**消息浏览器**对话框中，然后单击**确定**。  
  
3.  每条消息的消息上下文属性应可以看到**消息浏览器**对话框。  
