...
业务流程引擎使用名为 BTSNTSvc.exe.config 的 XML 文件来确定特定行为。 例如，冻结属性及其默认值配置为在 BTSNTSvc.exe.config 文件中的 XML 并读取所有主机实例包含业务流程开始时。 有关详细信息，请参阅[业务流程冻结和解除冻结](../core/orchestration-dehydration-and-rehydration.md)。  
  
 服务在启动时读取一次，此配置信息。 对它的任何更改将不会选取除非停止并重新启动此服务。  
...
## <a name="example-assembly-validation-only"></a>只验证示例： 程序集  
...
## <a name="example-remote-debugging-enabled"></a>示例： 启用进行远程调试  
...
## <a name="example-appdomain-configuration"></a>例如：AppDomain 配置  
 程序集分配给命名域使用赋值规则 （详见下文中）。 如果为某个程序集不指定任何规则，则该程序集将分配给一个特别域。 这样每个特别域分配的程序集的数目由 AssembliesPerDomain 的值确定。  
...
## <a name="throttling-messages-per-orchestration"></a>限制每个业务流程的消息  
 此属性中，btsntsvc.exe.config 文件中指定将阻止业务流程占用太多内存的限制可以具有的未处理消息的数量。 所有消息将都继续传送到 MessageBox;但是，排队的消息将传递到业务流程直到它处理某些其未处理的消息。  
  
 若要在 btsntsvc.exe.config 文件中指定此属性 (位于[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]根目录)，添加应用程序节点下的以下参数：  
...
```  
 ...
 在此示例中，一旦业务流程具有 100 个未完成的消息，MessageBox 将停止的发送其他消息。 未处理消息的业务流程的数量下降到 50 时，它将指定 MessageBox 可以继续发送消息。 可以指定其他值。  
  
 您还必须启用此功能，每个主机，在数据库中。 若要启用消息阻止的主机，必须编辑 dbo。应用程序在 BizTalkMsgBoxDb 数据库中的表。 对于每个你想要启用消息阻止每个业务流程的主机，设置 fAttributes 标志位为 1。 只有 fattribute 设置为 1 的那些主机将允许限制每个业务流程的消息。  
