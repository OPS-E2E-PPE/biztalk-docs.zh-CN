...
1. 登录到`https://login.salesforce.com/?lt=de`，使用 Salesforce 开发人员登录凭据。  
  
2. 在导航栏中，单击**机会**，然后单击**客户 1**。 已创建在这个机会[步骤 2:设置 Salesforce 系统](../core/step-2-set-up-the-salesforce-system.md)。  
  
3. 在中**机会详细信息**部分中，记下中的关联产品**产品 （标准）** 部分。 此节下你最终将插入到 SQL Server 数据库的值。 下**机会详细信息**部分中，单击**编辑**按钮，然后更改的值**阶段**字段**已结束-赢得**。 单击“保存” 。  
...
4. 在 SQL Server Management Studio 中，运行查询**OrderDetails**表来选择所有行。  
    ...
    请注意，它列出为其更改阶段的机会中列出的产品。  
  
    ![向机会添加产品](../core/media/bts-sf-add-product.gif "BTS_SF_Add_Product")  
  
   您可以看到中输入的记录**OrderDetails**表对应于在 Salesforce 中创建的一组给定的产品的销售机会。 可以通过创建新的机会，并将新产品与机会相关联来重复这些步骤。