...
---
# <a name="receivepipeline-receivelocation-node"></a>ReceivePipeline （ReceiveLocation 节点）
绑定文件的 ReceiveLocation 节点的 ReceivePipeline 节点包含有关与绑定文件一起导出的接收位置一起使用的接收管道的特定信息。  
...
|--------------|-------------------|-------------------|---------------------|----------------------|------------------|  
|“属性”|特性|xs:string|指定接收管道的名称。|可选|默认值：空|  
...
|Description|特性|xs:string|指定接收管道的说明。|不需要|默认值：空|