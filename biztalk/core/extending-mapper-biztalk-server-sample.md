...
2.  运行 Setup.bat 文件，该文件将执行以下操作：  
    ...
    3.  在属性网格中，选择**脚本**属性，然后单击省略号 （…） 按钮以配置 functoid 脚本。  
  
    4.  在**配置脚本 Functoid**对话框中，选择**脚本 Functoid 配置**，指定以下：  
    ...
    5.  从[!INCLUDE[btsVStudioNoVersion](../includes/btsvstudionoversion-md.md)]**文件**菜单上，选择**保存**到映射文件中，保存更改并关闭解决方案。  
...
5.  在**ExtendingMapperSP-发送端口属性**对话框中，单击**出站映射**页。  
  
     在**映射**列中，从下拉列表中，选择所需的映射，然后单击**确定**。 下表对这些映射进行了说明。  
  
    |“要应用的映射”属性|Description|  
    |---------------------------|-----------------|  
    |Microsoft.Samples.BizTalk.ExtendingMapper。 Scriptor_CallExternalAssembly|演示如何调用中从外部.NET 程序集的函数到**脚本**functoid 在地图中，基于此 functoid 的输入参数。 这样做有助于将任何处理逻辑与映射文件完全分离。 此映射文件使用本示例附带的 MapperClassLibrary.dll 程序集。|  
    |Microsoft.Samples.BizTalk.ExtendingMapper。 Scriptor_InlineScripts|演示如何编写简单的内联脚本内**脚本**functoid 使用.NET 语言，如 C#、 Visual Basic.NET 和 JScript.NET 映射文件中的。|  
    |Microsoft.Samples.BizTalk.ExtendingMapper。 Scriptor_GlobalVariableInInlineScript|演示如何使用中的内联脚本的全局变量**脚本**functoid。 全局变量通常用来跨不同保持映射文件中的状态信息**脚本**functoid。|  
    |Microsoft.Samples.BizTalk.ExtendingMapper。 Scriptor_InlineXslt|演示如何构造使用原始内联 XSLT 目标文档中的结构内**脚本**functoid 映射中的。 您可以构造目标文档使用的某些部分**脚本**functoid 与内联 XSLT 每当不能在 BizTalk 映射程序使用其他 functoid 执行该操作。|  
    |Microsoft.Samples.BizTalk.ExtendingMapper。 Scriptor_XsltCalltemplate|演示如何使用 XSLT 调用模板在目标文档中创建结构**脚本**functoid 映射中的。 XSLT 是调用模板可以接受参数，因此你可以创建结构的内联 XSLT 调用模板的优势根据输入参数**脚本**functoid。 您可以构造目标文档使用的某些部分**脚本**functoid 与内联 XSLT 每当不能在 BizTalk 映射程序使用其他 functoid 执行该操作。|  
    |Microsoft.Samples.BizTalk.ExtendingMapper。 Scriptor_InlineXsltCallingExternalAssembly|演示如何以调入外部.NET 程序集从内联 XSLT 内的**脚本**functoid 映射中的。 介绍可如何改写**自定义扩展 XML**带有自定义扩展文件包含要调用的外部.NET 程序集的详细信息的 ExternalAssembly_extxml.xml BizTalk 映射程序网格的属性。 您可以构造目标文档使用的某些部分**脚本**functoid 与内联 XSLT 每当不能使用其他 functoid 映射器 UI 在执行该操作。|  
    |Microsoft.Samples.BizTalk.ExtendingMapper。 OverridingMapXslt|演示如何使用自定义 XSLT 文件完全替代 BizTalk 映射器文件的已编译 XSLT。 你可以执行此操作通过重写**自定义 XSL 路径**属性和 （可选）**自定义扩展 XML** BizTalk 映射程序网格的属性。 你提供的自定义 XSLT 文件包含在要在运行时使用的项目的已编译 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] 程序集中。 在这种情况下将忽略映射文件 (.btm) 的内容。 此映射文件使用 OverridingMapXslt.xslt 和为 OverridingMapXslt.xml**自定义 XSL 路径**和**自定义扩展 XML**属性，分别。<br /><br /> 你可在解决方案资源管理器中对映射文件进行验证。 你可以然后将其用作模板文件，你可以编辑和用于**自定义 XSL 路径**BizTalk 映射程序网格的属性。 一旦无法使用 BizTalk 映射器生成 XSLT，你即可使用此选项。|  
