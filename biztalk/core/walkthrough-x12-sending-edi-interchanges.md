...
本演练提供了创建一个发送 EDI 解决方案的分步操作过程一交换使用[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]。  
...
2.  使用 EdiReceive 管道，接收端口将检查该消息并将其转换为 XML。 接收端口然后将测试消息放入 MeassageBox。  

3.  静态单向发送端口提取 XML 消息从 MessageBox。  

4.  静态单向发送端口验证针对消息架构的 EDI 消息，EDI 消息序列化为 EDI 交换，然后将 EDI 消息发送到贸易合作伙伴 Contoso 本地文件夹。  

## <a name="the-functionality-in-this-solution"></a>此解决方案中的功能  
 本演练使用以下功能：  

- 在本演练中，不测试接收确认。 若要了解如何接收确认，请参阅演示[演练 (X12):接收 EDI 交换并发送回确认](../core/walkthrough-x12--receive-edi-interchanges-and-send-back-an-acknowledgement.md)  
...
- EDI 类型和扩展的验证将传出交换上执行。  

- 使用 FILE 传输类型，该解决方案使用静态单向发送端口。  

  > [!NOTE]
  >  而不是静态单向发送端口，可以使用静态双向发送端口以将交换发送和接收确认。 此外可以使用动态单向发送端口以发送交换。 使用动态发送端口的详细信息，请参阅[配置为发送 EDI 交换和确认的动态发送端口](../core/configuring-a-dynamic-send-port-to-send-edi-interchanges-and-acknowledgments.md)。  

  > [!NOTE]
  >  可以使用 HTTP 适配器和 AS2 传输。 有关执行此操作的详细信息，请参阅[演练 (AS2):使用同步 MDN 通过 AS2 发送 EDI](../core/walkthrough-as2-sending-edi-over-as2-with-a-synchronous-mdn.md)或[演练 (AS2):使用异步 MDN 通过 AS2 发送 EDI](../core/walkthrough-as2-sending-edi-over-as2-with-an-asynchronous-mdn.md)。  

- 将启用 EDI 报告，并保存事务集以从交换状态报告进行查看。  

- 出于测试目的，该解决方案使用的接收位置用于接收测试消息。  

  下图显示了此解决方案中，使用静态单向发送端口的体系结构。  
...
- 将必需的消息架构添加到 BizTalk 项目，然后生成并部署该项目，使架构可供使用[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]在处理出站交换。  

- 创建接收端口和位置[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]接收 EDI 交换。 此接收位置与 Fabrikam 存放要发送到 Contoso 的 EDI 交换文件文件夹相关联。 接收位置将使用 EdiReceive 接收管道。  

- 创建发送端口的[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]向 Contoso 发送 EDI 交换。 在本演练中，将创建一个静态单向发送端口。  

- 为 Fabrikam 和 Contoso 创建参与方 （贸易合作伙伴）。  

- 为这两个贸易合作伙伴创建业务配置文件每个。  

- 创建通过配置要接收的消息的 EDI 属性的两个配置文件之间的协议。  
...
1. 在 Windows 资源管理器中，创建用于接收交换中的本地文件夹。  
...
7. 在控制台树中，选择**筛选器**，并输入要用于订阅消息的发送端口的筛选器表达式。 例如，可以使用接收位置将接收原始测试消息与筛选器表达式。 为此，对于**属性**，输入**BTS。ReceivePortName**; 对于**运算符**，输入**==**; 对于**值**输入到创建的接收端口的名称从 Fabrikam 接收 XML 消息。  

   > [!NOTE]
   >  您可以筛选所选的另一个属性，如 BTS。MessageType。  
...
2. 在 Windows 资源管理器中，打开为发送端口指定的目标文件夹。 验证该文件夹包含输出 EDI 交换的 ISA、 GS 和 ST 标头与协议属性中输入的值匹配。  
