---
title: "引擎性能和容量 |Microsoft 文档"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- capacity performance
- capacity engines
- performance, capacity
- performance, engines
- engines, capacity
- engines, performance
ms.assetid: 199d8696-09ee-4fd6-af14-bc384863df0d
caps.latest.revision: "4"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 14f5b9ef6bd313eb3d43132bd02d65156a870885
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/20/2017
---
# <a name="engine-performance-and-capacity"></a>引擎性能和容量
规划你的 BizTalk Server 解决方案时，引擎性能和容量是重要注意事项。 引擎性能随着时间的推移非常来处理文档的能力，而且引擎容量是你的 BizTalk Server 系统能够维护积压工作的已经接收但尚未处理的文档。  
  
## <a name="in-this-section"></a>本节内容  
  
-   [什么是可持续的性能？](../core/what-is-sustainable-performance.md)  
  
-   [引擎持久性和持久性](../core/engine-persistence-and-durability.md)  
  
-   [测量的最大可持续引擎吞吐量](../core/measuring-maximum-sustainable-engine-throughput.md)