...
在安装或升级 BAM 的过程中，默认情况下不启用加密。 若要启用加密，必须将 BAM 配置 XML 文件中的 UseEncryption 标志的值设置为 1。  
 ...
 还必须在 SQL Server Analysis Services 中启用加密。 有关启用加密的详细信息，请参阅[保护与 Analysis Services 实例的客户端通信](http://go.microsoft.com/fwlink/?LinkId=190805)(http://go.microsoft.com/fwlink/?LinkId=190805)。  
