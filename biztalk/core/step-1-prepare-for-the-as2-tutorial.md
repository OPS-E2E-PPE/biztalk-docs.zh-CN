...
---
# <a name="step-1-prepare-for-the-as2-tutorial"></a>步骤 1： 准备 AS2 教程
![步骤 1 11](../core/media/tut-step1-of-11.gif "Tut_Step1_of_11")  
  
 AS2 教程在一台计算机上运行。 若要准备本教程，你必须安装和配置[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]中所述[BizTalk Server 2013 和 2013 R2 的安装概述](http://msdn.microsoft.com/library/8041926c-cfc9-4eaf-9c28-a2c6e8015bc5)部分。 你还必须添加对本主题中所述的 BizTalk Server EDI 应用程序的引用。 AS2 教程所需的文件位于 [!INCLUDE[btsBiztalkServerPath](../includes/btsbiztalkserverpath-md.md)]SDK\AS2 Tutorial 文件夹中。  
...
1.  在 Windows 资源管理器，移动到[!INCLUDE[btsBiztalkServerPath](../includes/btsbiztalkserverpath-md.md)]SDK\AS2 教程\\_997ToFabrikam 文件夹。 右键单击\\_997ToFabrikam 文件夹，然后单击**属性**。  
  
2.  单击**安全**选项卡上，并依次**编辑**。 在**权限**对话框中，单击**添加**。  
  
3.  在**选择用户、 计算机、 服务帐户或组**对话框中，在对象名称窗格中，输入`Everyone`，然后单击**确定**。  
  
4.  选择**Everyone**中**组或用户名**框中，单击复选框**编写**(下**允许**列) 中**权限**窗格中，，然后单击**确定**。  
...
3.  在详细信息窗格中，右键单击你想要使用本教程中，然后选择的进程内主机**属性**。  
  
4.  在**主机属性**对话框中，在**常规**选项卡上，选择**仅限 32 位**，然后单击**确定**。  
  
5.  为独立主机重复步骤 3-4。  
  
 如果 BizTalk Server 安装在 64 位操作系统上，则在使用 32 位 BizTalk 主机进程时，还必须将 IIS 设置为在 32 位模式下运行。 设置 IIS 的说明存在于[步骤 5： 配置贸易合作伙伴网页](../core/step-5-configure-the-trading-partner-web-pages.md)，如 IIS，您可以设置上的 32 位工作进程每个应用程序池。  
...
## <a name="next-steps"></a>后续步骤  
 中所述部署示例 X12 架构[步骤 2： 创建并部署示例 X12 架构](../core/step-2-create-and-deploy-the-sample-x12-schema.md)  
