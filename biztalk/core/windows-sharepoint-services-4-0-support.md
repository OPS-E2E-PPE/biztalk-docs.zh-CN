...
-   消息发送到和来自 Windows SharePoint Services 4.0 Wiki 站点接收消息。  
 ...
 BizTalk Server 的 Windows SharePoint Services 适配器不是 Windows SharePoint Services 4.0 中提供以下功能提供支持：  
  
-   **回收站**: BizTalk Server 适配器的 Windows SharePoint Services 适配器不支持接收，或者从/到回收站显式发送消息。  
  
-   **列出文件夹**: BizTalk Server 的 Windows SharePoint Services 适配器可以将消息发送到列表，但它不能从列表中接收消息。 Windows SharePoint Services 4.0 支持文件夹中列出，但 BizTalk Server 的 Windows SharePoint Services 适配器不支持此功能。 因此，BizTalk Server 的 Windows SharePoint Services 适配器无法在根文件夹以外的列表文件夹中创建列表项。  
  
-   以下各节描述更详细介绍了如何使用 BizTalk Server 的 Windows SharePoint Services 适配器将消息发送到 Windows SharePoint Services 4.0 博客网站以及如何将消息发送到和从 Windows SharePoint Services 接收消息4.0 Wiki 站点。  
...
> [!IMPORTANT]
>  Windows SharePoint Services 4.0 Wiki 文档库支持版本控制，但是，BizTalk Server 2010does 的 Windows SharePoint Services 适配器不支持版本控制。 因此，将会为 BizTalk Server 更新由 Windows SharePoint Services 适配器的 Wiki 网页将失去他们以前的版本。 鉴于此限制，是为 BizTalk Server 接收由 Windows SharePoint Services 适配器并在不同的 Wiki 文档库存档的 Wiki 页将会保留只有其最后一个版本，与要删除的所有其他版本。  
