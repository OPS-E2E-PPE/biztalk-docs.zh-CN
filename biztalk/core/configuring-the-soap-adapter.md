---
title: "配置 SOAP 适配器 |Microsoft 文档"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- configuring [SOAP adapters]
- SOAP adapters, configuring
ms.assetid: 810f3543-21d3-4bb5-b477-6463f4576335
caps.latest.revision: "11"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 81d41cc925a68cc7aadfac53e82d622fe7d38461
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/20/2017
---
# <a name="configuring-the-soap-adapter"></a>配置 SOAP 适配器
本部分将介绍如何配置 SOAP 适配器。  
  
## <a name="in-this-section"></a>本节内容  
  
-   [如何配置 SOAP 接收处理程序](../core/how-to-configure-a-soap-receive-handler.md)  
  
-   [如何配置 SOAP 接收位置](../core/how-to-configure-a-soap-receive-location.md)  
  
-   [如何配置 SOAP 发送处理程序](../core/how-to-configure-a-soap-send-handler.md)  
  
-   [如何配置 SOAP 发送端口](../core/how-to-configure-a-soap-send-port.md)  
  
-   [SOAP 适配器配置和优化参数](../core/soap-adapter-configuration-and-tuning-parameters.md)  
  
-   [SOAP 适配器属性架构和属性](../core/soap-adapter-property-schema-and-properties.md)