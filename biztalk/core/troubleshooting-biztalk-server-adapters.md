---
title: "故障排除 BizTalk Server 适配器 |Microsoft 文档"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: 428b127f-df1e-4c41-9cc8-486519f15fa1
caps.latest.revision: "11"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: bdaf265f3a0c58f9505f9b4806dac0a5cee05a75
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/20/2017
---
# <a name="troubleshooting-biztalk-server-adapters"></a>故障排除的 BizTalk Server 适配器
本主题介绍常见疑难解答可用于确定和解决可能的集成 BizTalk 适配器中发生的问题的步骤。  
  
## <a name="in-this-section"></a>本节内容  
  
-   [故障排除的文件适配器](../core/troubleshooting-the-file-adapter.md)  
  
-   [故障排除 FTP 适配器](../core/troubleshooting-the-ftp-adapter.md)  
  
-   [故障排除 HTTP 适配器](../core/troubleshooting-the-http-adapter.md)  
  
-   [故障排除 MQSeries 适配器](../core/troubleshooting-the-mqseries-adapter.md)  
  
-   [故障排除 MSMQ 适配器](../core/troubleshooting-the-msmq-adapter.md)  
  
-   [故障排除 POP3 适配器](../core/troubleshooting-the-pop3-adapter.md)  
  
-   [故障排除 SMTP 适配器](../core/troubleshooting-the-smtp-adapter.md)  
  
-   [故障排除 SOAP 适配器](../core/troubleshooting-the-soap-adapter.md)  
  
-   [故障排除 Windows SharePoint Services Adapter](../core/troubleshooting-the-windows-sharepoint-services-adapter.md)  
  
-   [故障排除的 WCF 适配器](../core/troubleshooting-the-wcf-adapters.md)  
  
-   [使用 BizTalk 适配器跟踪](../core/using-biztalk-adapter-tracing.md)  
  
-   [使用 IPv6 寻址与 BizTalk 适配器](../core/using-ipv6-addressing-with-biztalk-adapters.md)