...
1.  单击**启动**，指向**所有程序**，单击[!INCLUDE[btsBizTalkServerStartMenuItemui](../includes/btsbiztalkserverstartmenuitemui-md.md)]，然后单击**跟踪配置文件编辑器**。  
...
3.  上**工具**菜单上，单击**应用跟踪配置文件**将.btt 文件应用到管理数据库设置从**设置管理数据库**上的菜单项**工具**菜单。 通过集成测试验证结果。  
...
> [!IMPORTANT]
>  通过命令行应用跟踪配置文件时，bttdeploy 会将跟踪配置文件应用到在运行配置向导时所指定的 BizTalk 管理数据库。 如果您在跟踪配置文件编辑器选项“设置管理数据库”中指定了其他数据库设置，则会使用该设置。 如果将管理服务器和数据库指定为 bttdeploy 的命令行选项，则该命令行选项将覆盖所有其他设置。 有关使用 bttdeploy 的详细信息，请参阅[跟踪配置文件部署实用工具](../core/tracking-profile-deployment-utility.md)。  
...
3.  上**工具**菜单上，单击**删除跟踪配置文件**删除基于.btt 文件从 BizTalk 管理数据库的跟踪配置文件。  
