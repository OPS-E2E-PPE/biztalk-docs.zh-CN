...
## <a name="which-versions-of-64-bit-windows-are-supported"></a>支持哪些版本的 64 位 Windows？  
 所有版本的[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]在支持的操作系统上支持 32 位执行和本机 64 位执行。 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]包括 32 位和 64 位配置选项。 
...
## <a name="is-itanium-based-hardware-supported"></a>是否支持基于 Itanium 的硬件？  
 BizTalk 运行时，没有。 对于 BizTalk 数据库，是。  
  
 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] 需要支持 AMD64 或 EM64T 的 CPU 硬件。 因此，[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]不支持在基于 Itanium 的 64 位 Cpu 上运行的 Windows 上。 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]未支持基于 Itanium 的运行[!INCLUDE[btsSQLServerNoVersion](../includes/btssqlservernoversion-md.md)]。 因而，Itanium 64 位 CPU 支持所有 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] 数据库。  
  
## <a name="which-biztalk-server-processes-run-in-64-bit-mode"></a>哪些 BizTalk Server 进程运行在 64 位模式下？  
 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]可执行文件位于在几个不同的服务器运行。 下表列出了其中[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]进程在 64 位模式下运行。  
...
##### <a name="biztalk-host-instances"></a>BizTalk 主机实例  
 BizTalk 主机是一个逻辑服务器组，每台服务器都称为一个主机实例。 每个主机实例部署为基于 BTSNTSvc.exe NT 服务。 业务流程和进程内适配器在加载和执行主机实例中。 可以将主机实例配置为使用在 32 位或 64 位模式中运行**仅限 32 位**中的复选框选项**主机属性**中的对话框[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]管理控制台。  
  
##### <a name="enterprise-sso"></a>企业 SSO  
 Microsoft 企业单一登录 (SSO) 运行于专用 NT 服务中 (ENTSSO.exe)。 它是在 32 位平台上的本机 32 位[!INCLUDE[btsWinNoVersion](../includes/btswinnoversion-md.md)]和 64 位上的本机 64 位[!INCLUDE[btsWinNoVersion](../includes/btswinnoversion-md.md)]。  
  
##### <a name="bam-portal-iis"></a>BAM 门户 (IIS)  
 业务活动监视 (BAM) 门户组件必须运行于使用 32 位 ASP.NET 3.5 的 IIS 中。 BAM 门户将以 WOW 模式在 64 位硬件上运行。 "在运行 64 位环境中的 BAM 门户"，请参阅[自定义 BAM 门户配置](../core/customizing-the-bam-portal-configuration.md)。  
 ...
 **Microsoft Visual Studio**  
 ...
 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]设计器可执行文件承载于 32 位内[!INCLUDE[btsVStudioNoVersion](../includes/btsvstudionoversion-md.md)]IDE。 [!INCLUDE[btsVStudioNoVersion](../includes/btsvstudionoversion-md.md)]支持使用 Microsoft 的 64 位项目开发[!INCLUDE[btsDotNetFramework](../includes/btsdotnetframework-md.md)]，前者可部署到[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]。  
 ...
 **Internet Explorer**  
 ...
 BAM 客户端需要安装和使用 64 位上的 32 位 Internet Explorer [!INCLUDE[btsWinNoVersion](../includes/btswinnoversion-md.md)]。  
  
## <a name="how-do-i-enable-native-64-bit-execution-of-orchestrations"></a>如何实现业务流程的本机 64 位执行？  
 将分配具有的主机实例中运行 orchestration**仅限 32 位**未选择的属性。 该主机实例必须运行在 Windows x64 计算机上。  
...
## <a name="will-net-framework-20-compiled-assemblies-jit-compile-properly-in-both-32-bit-and-64-bit"></a>.NET Framework 2.0 编译的程序集可否在 32 位和 64 位模式下正确进行 JIT 编译？  
 是。 如果程序集用编译[!INCLUDE[btsDotNetFramework](../includes/btsdotnetframework-md.md)]2.0 和**AnyCPU**标志，在 32 位或 64 位 Clr 中正确单个 DLL 将 JIT 编译。  
...
## <a name="will-32-bit-biztalk-server-executables-have-4gb-of-addressable-process-memory-on-windows-x64"></a>32 位 BizTalk Server 可执行文件是否在 Windows x64 上具有 4GB 可寻址进程内存？  
 是。 上[!INCLUDE[btsWinNoVersion](../includes/btswinnoversion-md.md)]x64，32 位 BTSNTSVC 和 IIS 进程在 WOW64 下运行，并可能利用完整 4 GB 的虚拟内存。 这对 32 位 Windows 上的 2GB 默认可寻址虚拟内存是一大改进。  
...
## <a name="which-adapters-are-capable-of-running-in-64-bit-mode"></a>哪些适配器能够运行在 64 位模式下？  
 默认情况下，所有适配器可以在 32 位模式下都运行在 32 位平台上[!INCLUDE[btsWinNoVersion](../includes/btswinnoversion-md.md)]和在 64 位上的 WOW64 上[!INCLUDE[btsWinNoVersion](../includes/btswinnoversion-md.md)]。 以下适配器可以运行在本机 64 位模式下（在 IIS 或 BTSNTSVC 作为主机进程的情况下）：  
...
> [!NOTE]
>  32 位和 64 位进程都支持 MQSeries 适配器。 该适配器具有在 Windows 的 IBM WebSphere MQ Server 上运行的 MQSeries 代理。 [为安装准备计算机](../install-and-config-guides/prepare-your-computer-for-installation.md)列出 MQ 要求。  
  
> [!NOTE]
>  如果你[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]应用程序连接到 Windows Server 2003 计算机 （如 SSO、 SQL 适配器、 MQSeries、 MQSAgent、 Oracle 和等等），然后安装[934016](http://support.microsoft.com/kb/934016)和[934849](http://support.microsoft.com/kb/934849)这些 Windows Server 上2003 服务器。  
...
## <a name="what-64-bit-sql-server-components-are-required-to-configure-bam-tools"></a>配置 BAM 工具需要哪些 64 位 SQL Server 组件？  
 配置向导是一个 32 位的过程;因此它需要某些组件允许它与 64 位进行通信[!INCLUDE[btsSQLServerNoVersion](../includes/btssqlservernoversion-md.md)]。 你必须安装以下 [!INCLUDE[btsSQLServerNoVersion](../includes/btssqlservernoversion-md.md)] 客户端组件后，才能实现 BAM 工具的配置：  
