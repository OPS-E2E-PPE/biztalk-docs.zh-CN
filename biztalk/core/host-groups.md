...
-   它必须是以下数据库中的 BTS_HOST_USERS SQL Server 角色的成员：  
    ...
    -   BizTalk 管理 (称为中的配置数据库[!INCLUDE[btsBizTalkServer2004](../includes/btsbiztalkserver2004-md.md)])  
...
> [!NOTE]
>  如果将某个主机指定为跟踪主机，对于跟踪数据库，BizTalk Server 管理将自动从 SQL Server 角色中删除 BizTalk 主机组。 对于 BizTalk 管理数据库和 MessageBox 数据库，必须手动从 SQL Server 角色中删除 BizTalk 主机组。 有关作为跟踪主机指定主机的信息，请参阅[如何修改主机属性](../core/how-to-modify-host-properties.md)。  
