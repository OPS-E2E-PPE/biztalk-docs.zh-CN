---
title: 管理 BAM |Microsoft 文档
ms.custom: ''
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: ''
ms.suite: ''
ms.tgt_pltfrm: ''
ms.topic: article
helpviewer_keywords:
- managing [BAM], about managing BAM
- managing [BAM]
ms.assetid: 2981a90d-aaa4-4991-beee-5ac1f12b5c9e
caps.latest.revision: 16
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 557fb97d0bbff5faa932091bbe5c195bcf479b9d
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/20/2017
ms.locfileid: "22262445"
---
# <a name="managing-bam"></a>管理 BAM
使用业务活动监视 (BAM)，业务用户可以实时查看关键业务绩效指标，这使他们可以更有效地工作。 业务用户可以使用此信息提高业务操作的速度和效率。 BAM 以一种各种交易一致的实时方式收集业务活动信息，而不论各种不同类型的环境的复杂性如何。  
  
## <a name="in-this-section"></a>本节内容  
  
-   [如何部署跟踪使用跟踪配置文件管理实用程序配置文件](../core/how-to-deploy-tracking-profiles-with-the-tracking-profiles-management-utility.md)  
  
-   [管理 BAM 事件总线服务](../core/managing-the-bam-event-bus-service.md)  
  
-   [更改 BAM 运行时设置](../core/changing-bam-runtime-settings.md)  
  
-   [更新 OLAP 和 RTA 连接字符串属性](../core/updating-olap-and-rta-connection-string-properties.md)  
  
-   [管理 BAM 动态基础结构](../core/managing-the-bam-dynamic-infrastructure.md)  
  
-   [管理 BAM 门户](../core/managing-the-bam-portal.md)  
  
-   [管理员的 BAM 任务](../core/bam-tasks-for-administrators.md)  
  
-   [如何重新生成实时数据工作簿](../core/how-to-regenerate-the-live-data-workbook.md)  
  
-   [如何更新实时数据工作簿的连接字符串](../core/how-to-update-the-connection-string-for-the-live-data-workbook.md)  
  
-   [管理远程活动的分布式的的导航](../core/managing-distributed-navigation-of-remote-activities.md)  
  
-   [如何在 BAM 中启用跟踪](../core/how-to-enable-tracing-in-bam.md)  
  
## <a name="see-also"></a>另请参阅  
 [在 BAM 中定义数据](../core/defining-data-in-bam.md)   
 [在 Excel 中定义的业务活动和视图](../core/defining-business-activities-and-views-in-excel.md)   
 [查看实时 BAM 数据](../core/viewing-live-bam-data.md)