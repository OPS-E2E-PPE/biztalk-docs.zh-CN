...
2.  展开[!INCLUDE[btsBizTalkServer2006r3ui](../includes/btsbiztalkserver2006r3ui-md.md)]**管理**，展开**BizTalk 组**，展开**应用程序**，然后展开发送端口所属应用程序。  
  
3.  单击**接收端口**文件夹。  
  
4.  在右侧窗格中，双击要配置的接收端口的名称。  
  
5.  接收端口的属性页上，在左窗格中，选择**常规**类别。  
  
6.  在右窗格中，选择**启用路由失败消息**复选框，并依次**应用**。  
...
5.  发送端口属性页上，在左窗格中，选择**传输高级选项**类别。  
  
6.  在右窗格中，在**传输选项**组框中，选择**启用路由失败消息**复选框，并依次**应用**。  
