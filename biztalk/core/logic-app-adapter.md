...
1. 在 BizTalk Server 管理中，展开**BizTalk Server 管理**，展开**BizTalk 组**，展开**应用程序**，然后展开你想为应用程序运行接收位置。 例如，展开**BizTalk 应用程序 1**。
...
8. **可选**。 在**绑定**选项卡上，配置任何超时和编码相关的基础的 WCF WebHttp 绑定的属性。 处理大消息时，这些属性是很有帮助。
    ...
    | 已接收消息的最大字节数 | 以字节为单位，包括消息头的消息，以在网络上接收输入的最大大小。 消息的大小受为每个消息分配的内存量。 你可以使用此属性来降低受拒绝服务 (DoS) 攻击的可能性。 <br/><br/>默认值：65536<br/>最大值：2147483647|
...
1. 在 BizTalk Server 管理中，展开**BizTalk Server 管理**，展开**BizTalk 组**，展开**应用程序**，然后展开你想为应用程序运行发送端口。 例如，展开**BizTalk 应用程序 1**。
