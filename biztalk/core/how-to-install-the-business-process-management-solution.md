...
##  <a name="step1"></a>准备计算机以安装业务流程管理解决方案  
  
#### <a name="to-prepare-the-computer-for-installing-the-business-process-management-solution"></a>若要准备计算机以安装业务流程管理解决方案  
  
1.  单击**启动**，指向**所有程序**，指向**管理工具**，然后单击**服务**。 使用**服务**控制台中，请确保以下服务正在运行：  
...
3.  如果你安装 Windows SharePoint Services，排除 （根） 的**Default Web Site**从，如下所示 Windows SharePoint Services 管理 Paths： 单击**启动**，指向**所有程序**，指向**管理工具**，然后单击**SharePoint 管理中心**。  
...
3.  打开命令提示符下，将当前目录更改到 %BTSSolutionsPath%\BPM\HistoryDB 文件夹中，类型`CreateDatabase.cmd`，然后按 ENTER 以创建历史记录数据库。  
...
5.  在命令提示符下，运行以下命令以创建 CSRWebApp Web 应用程序  
    ...
    -   `iisvdir /create "Default Web Site" CSRWebApp "%BTSSolutionsPath%\BPM\CSRWebApp"`  
        ...
        > [!NOTE]
        >  有关 iisvdir.vbs 的详细信息，请参阅 Microsoft TechNet 网站，网址[http://go.microsoft.com/fwlink/?LinkId=67830](http://go.microsoft.com/fwlink/?LinkId=67830)。  
...
6.  在命令提示符下，运行以下命令以为 OrderBroker_Proxy 创建新的 IIS 虚拟目录。  
    ...
    > [!NOTE]
    >  Internet 信息服务 (IIS) 管理器可用于创建 Web 应用程序。 有关如何在 IIS 7.0 中创建应用程序的详细信息，请参阅在 IIS 7.0 文档[http://go.microsoft.com/fwlink/?LinkId=59263](http://go.microsoft.com/fwlink/?LinkId=59263)。  
...
9. 设置**应用程序池**到你在上一步中创建的应用程序池。  
  
10. 重复前面的两个步骤的**CSRWebApp**应用程序。  
  
11. 重置 IIS 以确保所有这些更改立即生效。 若要执行此操作，运行**iisreset**在命令提示符。  
  
12. 在命令提示符下，将当前文件夹更改为 %btssolutionspath%\bpm\scripts，类型`CreateQueues.vbs`，然后按 enter 键以创建以下的专用队列。  
    ...
    |ToCSRSystemQ|是|HTTP|  
    |ToVendorSystemQ|是|HTTP|  
  
    > [!NOTE]
    >  你可以使用**计算机管理**管理单元以创建队列。 有关如何创建专用队列的详细信息，请参阅上的消息队列文档[http://go.microsoft.com/fwlink/?LinkId=59264](http://go.microsoft.com/fwlink/?LinkId=59264)。 有关如何安装 MSMQ 3.0 的详细信息，请参阅上的消息队列文档[http://go.microsoft.com/fwlink/?LinkId=59265](http://go.microsoft.com/fwlink/?LinkId=59265)。  
...
13. 在命令提示符下，将当前文件夹更改为 %btssolutionspath%\bpm\scripts，类型`CreateTestDirectories.cmd`，然后按 ENTER。  
    ...
    -   在 %SystemDrive%\Inetpub\ftproot 文件夹中创建 FromVendor 文件夹。  
        ...
        > [!NOTE]
        >  BizTalk 服务帐户必须具有到 FromVendor 文件夹的读/写权限。  
...
2.  使用业务规则引擎部署向导部署 Southridge Video 业务规则：  
    ...
    1.  单击**启动**，指向**所有程序**，指向[!INCLUDE[btsBizTalkServer2006r3ui](../includes/btsbiztalkserver2006r3ui-md.md)]，然后单击**业务规则引擎部署向导**。  
    ...
    5.  上**导入规则引擎策略/词汇文件**页上，单击**浏览**，在 %BTSSolutionsPath%\BPM\Rules 文件夹中，选择 DecodeAndValidateOrderRules.xml 文件，然后单击**下一步**。  
  
    6.  上**准备**页上，单击**下一步**，然后在**导入策略/词汇**页上，单击**下一步**  
  
    7.  在完成页上，选择**再次运行向导**以再次打开该向导，然后单击**完成**。  
    ...
    11. 上**部署策略**页上，选择**DecodeAndValidateOrder 1.0**中**规则引擎策略**下拉列表，然后单击**下一步**.  
  
    12. 上**准备**页上，单击**下一步**，然后在**部署策略**页上，单击**下一步**。  
  
    13. 在完成页上，单击**完成**。  
  
3.  如果你安装 BPM 解决方案在 64 位计算机上，然后  
  
    1.  打开请按照 32 位命令提示符： 单击**启动**，单击**运行**，类型`%SYSTEMROOT%\SYSWOW64\CMD.EXE`，然后按 ENTER。  
    ...
    4.  在 32 位命令提示符处，键入类型`DeployBPM.cmd`，然后按 ENTER。  
...
5.  单击**启动**，指向**所有程序**，指向[!INCLUDE[btsBizTalkServer2006r3ui](../includes/btsbiztalkserver2006r3ui-md.md)]，然后单击**BizTalk Server 管理**。  
  
    1.  在**BizTalk Server 管理控制台**，展开**BizTalk 组**，展开**应用程序**，展开**BTSScn.BPM.OrderBrokerApp**，展开**接收位置**，右键单击**供应商 RP RL**，然后单击属性。  
  
    2.  上**属性**对话框中，单击**配置**，，然后下表作为输入值**传输属性**对话框中：  
...
## <a name="next-steps"></a>后续步骤  
 测试的业务管理解决方案中的工作方式[如何运行业务流程管理解决方案](../core/how-to-run-the-business-process-management-solution.md)。  
