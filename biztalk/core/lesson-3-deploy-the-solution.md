...
---
# <a name="lesson-3-deploy-the-solution"></a>第 3 课：部署解决方案
部署 EAISolution 的第一步是将程序集添加到 BizTalk 管理数据库和全局程序集缓存中。  
  
 第二步是配置并启动应用程序。  
  
 在[第 2 课： 定义业务流程](../core/lesson-2-define-the-business-process.md)，你添加到 eai 进程业务流程的逻辑端口。 在本课中，您将定义物理端口并将物理端口绑定到逻辑端口。 您还必须将业务流程、接收端口和发送端口分配给主机。  主机是 BizTalk 项目的虚拟容器。  每个主机都已指定主机实例来处理项目。  
  
 在本课中，您将了解到如何通过使用 BizTalk Server 管理控制台管理 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] 项目。 有关使用 BizTalk Server 管理控制台的信息，请参阅[使用 BizTalk Server 管理控制台](../core/using-the-biztalk-server-administration-console.md)。  
