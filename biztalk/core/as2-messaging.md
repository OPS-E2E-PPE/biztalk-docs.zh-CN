---
title: "AS2 消息传送 |Microsoft 文档"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: b8a95b88-1225-46db-b5e5-50382ed74653
caps.latest.revision: "8"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: eab24751847748f0a86724f755fb18648e072d42
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/20/2017
---
# <a name="as2-messaging"></a>AS2 消息传送
本部分介绍 AS2 和消息处置通知 (MDN) 消息。 这些主题包含的信息将有助于您了解 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] 是如何处理 AS2 和 MDN 消息的。 有关 MDN 和 AS2 消息的完整信息，请参阅[RFC 4130"MIME 基于安全对等业务数据交换使用 HTTP，适用性语句 2 (AS2)](http://go.microsoft.com/fwlink/?LinkID=184212) ([http://go.microsoft.com/fwlink/?LinkID=184212](http://go.microsoft.com/fwlink/?LinkID=184212))。  
  
## <a name="in-this-section"></a>本节内容  
  
-   [AS2 消息](../core/as2-messages.md)  
  
-   [MDN 消息](../core/mdn-messages.md)  
  
## <a name="see-also"></a>另请参阅  
 [AS2 解决方案体系结构](../core/as2-solution-architecture.md)