...
## <a name="explanation"></a>解释  
 此警告事件表明，未在主密钥服务器的注册表中找到任何密钥。 SSO 主密钥以加密形式存储在 SSO 主密钥服务器的注册表中。 两个机密通常保存在注册表： 当前的主密钥和上一个主密钥。 密钥使用 GUID（主密钥 ID）标识。 如果请求指定的主密钥时无法在注册表中找到该主密钥，则将返回此错误代码。  
