---
title: "使用 SSO |Microsoft 文档"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- managing [SSO]
- SSO, managing
ms.assetid: e7245632-9c71-4b1f-836d-a4ea1dd6e5ee
caps.latest.revision: "12"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 850425f140b526baf251568f09ab47db9115e8ad
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/20/2017
---
# <a name="using-sso"></a>使用 SSO
您可以使用 MMC 管理单元或命令行管理实用工具 (ssomanage) 来管理 SSO 系统。 其中的活动包括：更新 SSO 数据库，添加、删除和管理应用程序，以及管理用户映射。  
  
> [!NOTE]
>  在支持用户帐户控制 (UAC) 的系统上，可能需要具有管理权限才能运行 SSO 命令行工具。  
  
 只有单一登录管理员才可以执行这些任务。  
  
## <a name="in-this-section"></a>本节内容  
  
-   [如何设置 SSO 服务器](../core/how-to-set-the-sso-server.md)  
  
-   [如何启用 SSO](../core/how-to-enable-sso.md)  
  
-   [如何更改主密钥服务器](../core/how-to-change-the-master-secret-server.md)  
  
-   [如何禁用 SSO](../core/how-to-disable-sso.md)  
  
-   [如何更新 SSO 数据库](../core/how-to-update-the-sso-database.md)  
  
-   [如何显示的 SSO 数据库信息](../core/how-to-display-the-sso-database-information.md)  
  
-   [如何配置 SSO 票证](../core/how-to-configure-the-sso-tickets.md)  
  
-   [如何审核 SSO](../core/how-to-audit-sso.md)  
  
-   [如何启用 SSO 的 SSL](../core/how-to-enable-ssl-for-sso.md)  
  
-   [管理主密钥](../core/managing-the-master-secret.md)  
  
-   [如何指定 SSO Administrators 和 Affiliate Administrators 帐户](../core/how-to-specify-sso-administrators-and-affiliate-administrators-accounts.md)  
  
-   [管理关联应用程序](../core/managing-affiliate-applications.md)  
  
-   [管理用户映射](../core/managing-user-mappings.md)  
  
-   [主机启动的 SSO](../core/host-initiated-sso.md)