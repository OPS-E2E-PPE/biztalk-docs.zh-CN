...
---
# <a name="configuring-charset-and-separators-edifact"></a>配置字符集和分隔符 (EDIFACT)
在合作伙伴协议中，您可以指定字符集 (UNA)[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]将用于创建为传出 EDIFACT 消息信封时验证参与方属性。 此外可以指定将交换中的分段使用哪些分隔符和终止符 (UNB)。  
