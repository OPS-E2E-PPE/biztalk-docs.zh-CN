---
title: BizTalk Server 如何接收 AS2 消息 |Microsoft 文档
ms.custom: ''
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: ''
ms.suite: ''
ms.tgt_pltfrm: ''
ms.topic: article
ms.assetid: ccfb4f6c-1fec-4af5-99e5-12634071dd3a
caps.latest.revision: 10
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 51f4b28397aa636b36ec3d063e441d9479e16cd2
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/20/2017
ms.locfileid: "22246637"
---
# <a name="how-biztalk-server-receives-as2-messages"></a>BizTalk Server 如何接收 AS2 消息
本部分介绍 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] 中的 AS2 组件如何处理传出 AS2 消息。  
  
## <a name="in-this-section"></a>本节内容  
  
-   [AS2 接收组件](../core/as2-receive-components.md)  
  
-   [传入 AS2 消息的的协议解析](../core/agreement-resolution-for-incoming-as2-messages.md)  
  
-   [接收方在处理传入 EDI 消息通过 AS2](../core/receive-side-processing-of-an-incoming-edi-message-over-as2.md)  
  
-   [接收方在处理传入通过 AS2 非 EDI 消息](../core/receive-side-processing-of-an-incoming-non-edi-message-over-as2.md)  
  
-   [处理传入 AS2 消息](../core/processing-an-incoming-as2-message.md)  
  
-   [生成传出 MDN](../core/generating-an-outgoing-mdn.md)  
  
-   [处理传入的 MDN](../core/processing-an-incoming-mdn.md)  
  
## <a name="see-also"></a>另请参阅  
 [AS2 解决方案体系结构](../core/as2-solution-architecture.md)