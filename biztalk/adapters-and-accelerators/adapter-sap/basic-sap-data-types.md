...
|D (日期：YYYYMMDD)*|RFC_DATE|xsd:dateTime|DateTime|在内部，该适配器反序列化到值**DateTime**对象。 然后，调用**DateTime.ToUniversalTime**方法以将此对象的值转换为 UTC。 最后，日期部分 (**DateTime.Date**) 用于创建发送到 SAP 系统的值。 SAP 系统将此日期值作为本地时间。<br /><br /> 您应将日期值指定为 UTC，以避免转换。<br /><br /> -对于化，建议使用以下模式:"(\d\d\d\d-\d\d-\d\d)T(00:00:00) (。\*)Z"。<br />-对于**DateTime**对象集**DateTime.Kind**到**DateTimeKind.Utc**。|  
...
|ABAP 数据类型|RFC 类型|XSD 类型|.NET 类型|格式字符串|  
|--------------------|--------------|--------------|---------------|-------------------|  
|D (日期：YYYYMMDD)|RFC_DATE|xsd:string|String|SAP 日期格式：YYYYMMDD。<br /><br /> 允许使用字符的日期数字，因此该值是实质上是八个字符的字符串|  
|T (时间：HHMMSS)|RFC_TIME|xsd:string|String|SAP 时间格式：为 HHMMSS。<br /><br /> 允许使用字符时间位数字，因此该值是实质上是六个字符字符串|  
