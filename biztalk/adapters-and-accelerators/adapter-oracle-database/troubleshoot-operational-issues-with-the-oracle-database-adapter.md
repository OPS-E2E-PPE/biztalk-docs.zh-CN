...
## <a name="enable-tracing"></a>启用跟踪  
 有关跟踪中的支持信息[!INCLUDE[adapteroracle_short](../../includes/adapteroracle-short-md.md)]，请参阅[诊断跟踪和消息日志记录为 Oracle 数据库适配器](../../adapters-and-accelerators/adapter-oracle-database/diagnostic-tracing-and-message-logging-for-the-oracle-database-adapter.md)。  
  
## <a name="known-issues"></a>已知问题  
 以下是使用时可能遇到的最常见错误[!INCLUDE[adapteroracle_short](../../includes/adapteroracle-short-md.md)]，以及其可能的原因和解决方法。   
  
### <a name="BKMK_OraDBLoading"></a>在加载适配器绑定错误  
 **问题**  
...
-   请确保在你安装的计算机上安装了所需的 LOB 客户端版本[!INCLUDE[adapterpacknoversion](../../includes/adapterpacknoversion-md.md)]。 [支持的业务线 (LOB) 和企业系统](https://social.technet.microsoft.com/wiki/contents/articles/17631.biztalk-server-supported-line-of-business-lob-and-enterprise-systems.aspx)列出支持的客户端版本。  
...
###  <a name="BKMK_OraDBAdapDisplay"></a>Oracle 数据库适配器不会显示在列表中在 BizTalk Server 管理控制台中的适配器  
 **问题**  
  
[!INCLUDE[adapteroracle_short](../../includes/adapteroracle-short-md.md)]与 inlcuded [!INCLUDE[adapterpacknoversion](../../includes/adapterpacknoversion-md.md)] BizTalk Server 管理控制台中的适配器的列表中未列出。  
  
 **可能的原因**  
  
 [!INCLUDE[adapteroracle_short](../../includes/adapteroracle-short-md.md)]是 WCF 自定义绑定。 因此，尽管[!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]管理控制台显示[!INCLUDE[wcfadapter_short](../../includes/wcfadapter-short-md.md)]，它不会显示 WCF 自定义绑定，并因此，不会显示基于 WCF 的[!INCLUDE[adapteroracle_short](../../includes/adapteroracle-short-md.md)]。  
  
 **解决方法**  
  
 你可以显式添加[!INCLUDE[adapteroracle_short](../../includes/adapteroracle-short-md.md)]到[!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]管理控制台中所述的步骤[将 Oracle 数据库适配器添加到 BizTalk Server 管理控制台](../../adapters-and-accelerators/adapter-oracle-database/adding-the-oracle-database-adapter-to-biztalk-server-administration-console.md)。  
  
###  <a name="BKMK_OraDBXMLRetrieve"></a>检索具有多个 65,536 节点的 XML 输出时出错  
 **问题**  
  
 在检索 XML 输出，有多个 65,536 节点时，该适配器将报告以下错误。  
 ...
 **解决方法**  
 ...
 可以通过设置来解决此问题`maxItemsInObjectGraph`参数。 你可以设置这在以下两种方法之一：  
 ...
 **解决方法**  
 ...
 指定的 SOAP 操作中发送端口或 BizTalk 业务流程中的消息上下文属性。 有关说明，请参阅[配置 Oracle 数据库的 SOAP 操作](../../adapters-and-accelerators/adapter-oracle-database/configure-the-soap-action-for-oracle-database.md)。 请参阅[消息和消息架构](messages-and-message-schemas-for-biztalk-adapter-for-oracle-database.md)若要查看的每个操作的操作的列表。  
...
```  
 ...
 在以上格式中，操作名称受生成架构时选择该操作。 例如，如果你生成 Insert 操作对表的架构，操作中的操作名称将"插入"。 但是中的逻辑端口的操作名称创建在 BizTalk 业务流程中[!INCLUDE[btsVStudioNoVersion](../../includes/btsvstudionoversion-md.md)]可能不同。  
 ...
 **问题**  
 ...
 [!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]指定连接 URI 来连接到 Oracle 数据库时，则报告以下错误。  
...
###  <a name="BKMK_OraDBInvalCursor"></a>无效的游标异常时调用采用 REF CURSOR 参数的存储的过程  
 **问题**  
  
 在调用时采用 REF CURSOR 输入 Oracle 数据库中的过程，你可能会出现以下异常：  
 ...
 **可能的原因**  
 ...
 正在调用的过程的 PL/SQL 块无法管道 REF Cursor，即，在 REF CURSOR 无法获取分配给出 REF CURSOR。  
  
 **解决方法**  
  
 PL/SQL 块必须不通过管道传递到而不正确处理 OUT REF Cursor IN。  
  
###  <a name="BKMK_OraDBValidateResp"></a>验证使用 BizTalk Server ReadLOB 操作的响应时出错。  
 **问题**  
  
 执行 ReadLOB 操作使用时[!INCLUDE[adapteroracle_short](../../includes/adapteroracle-short-md.md)]与[!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]，从 Oracle 数据库响应未通过验证对 Web 服务描述语言 (WSDL)。  
  
 **可能的原因**  
  
 WSDL 包含的程序定义执行的基于服务的请求，但不是需要为 BizTalk 方案 StreamBody 节点名称。 因此，输出 XML，不包含 StreamBody 节点，相比的 WSDL 中，验证将失败。  
  
 **解决方法**  
  
 使用 BizTalk Server 生成的输出进行验证时，请从 WSDL 删除 StreamBody 节点。 执行以下步骤，若要这样做：  
...
2.  从 WSDL 删除以下。  
    ...
    > [!NOTE]
    >  不支持 ReadLOB 操作[!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]。 应使用表选择操作将从中读取 LOB 数据[!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]解决方案。  
  
###  <a name="BKMK_OraDBSchemaValidateFail"></a>架构验证在轮询的情况下可能会失败  
 **问题**  
  
 在方案中的架构验证失败其中[!INCLUDE[adapteroracle_short](../../includes/adapteroracle-short-md.md)]轮询数据库包含 ROWID 或 UNROWID 类型的字段的表。  
  
 **可能的原因**  
  
 在设计时时适配器生成包含 ROWID 或 UNROWID，类型的字段的表的元数据架构包括"nillable = false"，这意味着 ROWID 或 UNROWID 类型的字段不能为 null。 但是，在运行时时该适配器中检索元数据，ROWID 或 UNROWID 类型的字段包含 null 值。 因此架构验证将失败。  
  
 **解决方法**  
  
 如果你使用[!INCLUDE[adapteroracle_short](../../includes/adapteroracle-short-md.md)]与[!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]，你可以选择禁用架构验证。 或者，您可以手动编辑架构更改"nillbale = true"对于 ROWID 和 UNROWID 数据类型。  
 ...
 **解决方法**  
 ...
 如果记录类型 LONG 数据类型，您必须显式定义它作为包的一部分。  
  
###  <a name="BKMK_OraDBAdapRecognize"></a>适配器无法识别的物理端口上的操作，即使使用适配器服务外接程序生成的绑定文件用于创建端口  
 **问题**  
  
 在使用后[!INCLUDE[consumeadapterservshort](../../includes/consumeadapterservshort-md.md)]若要生成对 Oracle 数据库的特定操作的架构外, 接程序还会创建端口绑定文件。 可以导入此文件使用绑定的[!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]BizTalk Server 中创建的物理端口的管理控制台。 但是，当消息发送到使用此类端口 Oracle 数据库时，该适配器将无法了解在端口上指定的操作，并提供了类似于以下错误：  
 ...
 **可能的原因**  
 ...
 在创建逻辑端口 BizTalk 业务流程中时，指定这些端口上的操作的某些名称，或只需使用如 Operation_1、 Operation_2 等的默认名称。但是，在生成的绑定文件[!INCLUDE[consumeadapterservshort](../../includes/consumeadapterservshort-md.md)]，操作名称是与为其生成元数据的 Oracle 数据库操作的名称相同。 例如，如果 Oracle 数据库中生成 ACCOUNTACTIVITY 表选择操作的元数据，操作将设置为以下：  
...
-   对为其生成元数据，例如选择操作从 Operation_1 等更改 BizTalk 业务流程中的逻辑端口中的操作名称。  
...
###  <a name="BKMK_OraDBOverflowExcep"></a>适配器引发溢出异常 ("System.OverflowException") 上执行操作  
 **问题**  
...
###  <a name="BKMK_OraDBRootNode"></a>与 RootNode TypeName BizTalk 项目中的错误  
 **问题**  
...
###  <a name="BKMK_OraDBInvalBinding"></a>使用 Visual Studio 中的适配器时出现的无效的绑定警告  
 **问题**  
 ...
 **可能的原因**  
 ...
 由于会出现此警告[!INCLUDE[adapteroracle_short](../../includes/adapteroracle-short-md.md)]绑定， `oracleDBBinding`，不标准绑定随[!INCLUDE[firstref_btsWinCommFoundation](../../includes/firstref-btswincommfoundation-md.md)]。  
  
 **解决方法**  
  
 可以安全地忽略此警告。  
  
###  <a name="BKMK_OraDBNotification"></a>BizTalk Server 引发异常，如果你在同一应用程序使用多个通知架构或跨同一个主机上的多个应用程序使用通知架构  
 **问题**  
...
[解决与 Oracle 数据库适配器的安装问题](../../adapters-and-accelerators/adapter-oracle-database/troubleshoot-installation-issues-with-the-oracle-database-adapter.md)