...
使用是的成员的帐户登录[!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]管理员或 BizTalk 操作员组。 请参阅[用于部署和管理 BizTalk 应用程序所需权限](../../core/permissions-required-for-deploying-and-managing-a-biztalk-application.md)，和[最低安全权限](https://social.technet.microsoft.com/wiki/contents/articles/24590.minimum-security-rights-for-biztalk-server-2006-to-2016.aspx)有关权限的指南。
...
4.  右键单击**发送端口**，指向**新建**，并指向一种你想要根据之间的通信模式配置的端口[!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]和 Oracle 数据库。  
...
7.  在**WCF 自定义传输属性**对话框框中，执行以下操作：  
    ...
    3.  单击**绑定**选项卡上，并从**绑定类型**下拉列表中，选择**oracleDBBinding**。 你可以指定不同的绑定属性公开的[!INCLUDE[adapteroracle_short](../../includes/adapteroracle-short-md.md)]。 有关绑定属性的详细信息，请参阅[阅读有关 Oracle 数据库适配器绑定属性](../../adapters-and-accelerators/adapter-oracle-database/read-about-the-oracle-database-adapter-binding-properties.md)。  
    ...
    4.  单击**凭据**选项卡，执行下列操作之一：  
        ...
        -   选择**使用单一登录**选项，然后指定关联 SSO 应用程序。  
         ...
         有关与 BizTalk Server 安全性的详细信息，请参阅[安全性与 Oracle 数据库适配器和 BizTalk Server](../../adapters-and-accelerators/adapter-oracle-database/security-with-the-oracle-database-adapter-and-biztalk-server.md)。  
  
         若要返回到**发送端口属性**对话框中，单击**确定**。  
  
8.  从**发送处理程序**下拉列表中，选择**BizTalkServerApplication**。  
  
9. 如果你选择了**静态单向发送端口**在步骤 4 中，指定发送管道。 从**发送管道**下拉列表中，选择对应于 XMLTransmit 管道。  
...
## <a name="deploy-adapters-for-receiving-messages-from-an-oracle-database"></a>部署用于从 Oracle 数据库中接收消息的适配器  
...
4.  右键单击**接收端口**，指向**新建**，然后单击**单向接收端口**或**请求响应接收端口**，具体取决于BizTalk Server 和 Oracle 数据库之间的通信模式。  
...
8.  在**WCF 自定义传输属性**对话框框中，执行以下操作：  
    ...
    3.  单击**其他**选项卡，然后执行下列操作之一：  
        ...
        -   选择**用户帐户**，并指定的用户名和密码以连接到 Oracle 数据库。  
