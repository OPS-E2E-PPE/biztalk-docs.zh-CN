...
---
# <a name="reuse-oracle-database-adapter-bindings"></a>重复使用 Oracle 数据库适配器绑定
绑定创建一个逻辑终结点 （例如业务流程端口或角色链接） 和物理终结点之间的映射 (如发送和接收端口)。 这样，BizTalk 业务解决方案的不同组件之间的通信。 可以使用 BizTalk Server 管理控制台来创建绑定。  
...
有关指定用户名和密码的信息，请参阅[配置 Oracle 数据库的登录凭据](../../adapters-and-accelerators/adapter-oracle-database/configure-the-sign-in-credentials-for-the-oracle-database.md)。  
