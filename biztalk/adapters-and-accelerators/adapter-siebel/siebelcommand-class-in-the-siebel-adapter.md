...
在建立与 Siebel 系统的连接后[!INCLUDE[adoprovidersiebelshort](../../includes/adoprovidersiebelshort-md.md)]分析 Siebel 命令字符串和 ADO.NET 客户端提供的命令参数并将该命令映射到 WCF 请求消息。 [!INCLUDE[adoprovidersiebelshort](../../includes/adoprovidersiebelshort-md.md)]然后将请求发送到[!INCLUDE[adaptersiebel_short](../../includes/adaptersiebel-short-md.md)]，并获取响应 XML 和正文内容的适配器。 [!INCLUDE[adoprovidersiebelshort](../../includes/adoprovidersiebelshort-md.md)]然后使用`XMLDataReader`从 XML 正文检索关系数据。  
 ...
 使用的实例`Microsoft.Data.SiebelClient.SiebelClientFactory`，客户端程序可以获取的实例`System.Data.Common.DbCommand`类来构造一个 Siebel 命令。  
...
|----------|--------------|-----------------|  
|**DbConnection**|获取和设置|此文件应包含基础`DbConnection`实例从此`DbCommand`获取实例。|  
|**DbParameterCollection**|获取|获取的集合`DbParameter`对象。|  
  
 `SiebelCommand`此外支持以下`DbCommand`*公共*属性：  
  
|Name|Get/Set|Description|  
|----------|--------------|-----------------|  
|**CommandText**|获取和设置|这包含 ADO.NET 客户端想要执行的 SQL 语句。|  
...
|**ExecuteDbDataReader**|这将执行的 SELECT 和 EXEC 命令并返回`DbDataReader`。|  
  
 `SiebelCommand`此外支持以下`DbCommand`*公共*方法：  
  
|Name|Description|  
|----------|-----------------|  
|**CreateParameter**|创建一个新`DbParameter`通过实例`CreateDbParameter().`|  
