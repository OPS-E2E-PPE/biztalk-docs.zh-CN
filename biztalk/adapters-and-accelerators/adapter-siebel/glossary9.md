...
**metadata**  
在 WCF 中，是指由服务公开的协定说明。 这称为服务说明和表示 WSDL 文档中。 由适配器公开的元数据描述的 （接口） 它可以对基础 LOB 系统执行的操作。 信息，如位置、 时间、 消息大小和/或异常信息。
...
**Siebel 业务服务**  
业务服务方法或函数，可以直接调用 Siebel 系统中的集合。
  
**Siebel COM 数据控件库**  
包含启用外部客户端，如 Siebel 适配器来建立连接和通信使用 Siebel 应用程序对象管理器在 Siebel 服务器上的接口。
...
**WS-元数据交换 (MEX) 终结点**  
实现 WCF 服务，如适配器公开的终结点**IMetadataExchange**接口。 WS-元数据交换终结点可用于检索服务说明 (WSDL) 的公开目标系统上的适配器的操作。 |  
