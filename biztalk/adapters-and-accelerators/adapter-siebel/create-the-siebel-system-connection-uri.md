...
[!INCLUDE[adaptersiebel](../../includes/adaptersiebel-md.md)]连接 URI 包含适配器用于建立到 Siebel 系统的连接的属性。  
 ...
 本主题提供有关 Siebel 连接 URI 的信息，并还提供了指向其他介绍如何在不同的编程方案中指定连接 URI 的主题的链接。  
  
## <a name="connection-uri-for-the-siebel-adapter"></a>为 Siebel 适配器的连接 URI  
...
```  
 ...
 下列各节描述 Siebel 连接 URI 的每个组件实现的属性。  
...
### <a name="user-information-in-the-siebel-connection-uri"></a>Siebel 连接 URI 中的用户信息  
 默认情况下， [!INCLUDE[adaptersiebel_short](../../includes/adaptersiebel-short-md.md)] Siebel 系统凭据指定连接 URI 中时引发异常。 这是因为这些凭据表示为纯文本，会带来的固有安全风险。 你可以设置**AcceptCredentialsInUri**是否连接 URI 可以包含凭据属性绑定到控件。 如果**AcceptCredentialsInUri**属性是**false**、[!INCLUDE[adaptersiebel_short](../../includes/adaptersiebel-short-md.md)]引发异常，如果连接 URI 包含凭据; 如果属性是**true**，不引发异常。  
  
> [!IMPORTANT]
>  由于通过将凭据作为纯文本传递字符串中带来的固有安全风险，最好不在连接 URI 中指定 Siebel 系统凭据。  
  
 有多种提供 Siebel 系统凭据，而在连接 URI 中指定它们。  
  
-   在代码中，你可以设置**ClientCredentials**上适当的对象的属性。  
  
-   当你使用[!INCLUDE[addadapterservrefshort](../../includes/addadapterservrefshort-md.md)]或[!INCLUDE[consumeadapterservshort](../../includes/consumeadapterservshort-md.md)]，你可以通过选择输入凭据**安全**选项卡**配置适配器**对话框。  
  
-   当你指定发送端口或接收中的位置绑定[!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]解决方案，你可以通过选择输入凭据**安全**相应对话框中的选项卡。  
  
 Siebel 连接 URI 表示为所需的用户身份验证参数的名称值集合中的用户信息 (userinfoparams)。 下表介绍了这些参数。  
  
|属性|Description|  
|--------------|-----------------|  
|用户名|Siebel 系统; 上的用户名此值是区分大小写。 必须设置**AcceptCredentialsInUri**属性绑定到**true**在连接 URI 中指定用户名和密码。 **注意：** [!INCLUDE[adaptersiebel_short](../../includes/adaptersiebel-short-md.md)]保留在打开 Siebel 系统上的连接时输入的用户名的值的大小写。|  
|密码|Siebel 系统; 上的用户密码此值是区分大小写。 必须设置**AcceptCredentialsInUri**属性绑定到**true**在连接 URI 中指定用户名和密码。 **注意：** [!INCLUDE[adaptersiebel_short](../../includes/adaptersiebel-short-md.md)]保留在打开 Siebel 系统上的连接时输入的密码的值的大小写。|  
  
### <a name="host-information-in-the-siebel-connection-uri"></a>Siebel 连接 URI 中的主机信息  
 Siebel 主机信息 (hostinfoparams) 指定 Siebel 系统的地址采用以下格式: [服务器]: [端口]。 根据 Siebel 服务器版本，Siebel 主机信息采用不同的值：  
  
-   **为 Siebel 7.5 和更早版本的版本**，主机信息参数采用的计算机上安装哪个 Siebel 网关服务器和 Siebel 网关端口号的名称。  
  
-   **为 Siebel 7.7 及更高版本**，主机信息参数将在其安装服务器 Siebel 和 Siebel 连接 broker 端口号的计算机的名称。  
  
    > [!IMPORTANT]
    >  当你使用[!INCLUDE[addadapterservreflong](../../includes/addadapterservreflong-md.md)]或[!INCLUDE[consumeadapterservlong](../../includes/consumeadapterservlong-md.md)]若要连接到 Siebel 系统，主机信息必须提供"SiebelGateway"连接属性。  
...
|Transport|传输;仅支持 tcpip。 此参数可选。 如果未指定，则 Siebel 系统将提供默认值 (tcpip)。|  
...
## <a name="using-reserved-characters-in-the-connection-uri"></a>在连接 URI 中使用保留的字符  
 [!INCLUDE[adaptersiebel_short](../../includes/adaptersiebel-short-md.md)]不支持指定的连接具有任何参数值的特殊字符的 URI。 连接参数值包含特殊字符，请确保执行下列任一操作：  
...
> [!NOTE]
>  此示例 URI 包含 Siebel 系统凭据;必须设置**AcceptCredentialsInUri**属性绑定到**true**用于连接 URI，其中包含的凭据。  
  
 有关如何建立到 Siebel 系统 （包括设置连接属性） 的连接时你：  
  
-   使用[!INCLUDE[consumeadapterservlong](../../includes/consumeadapterservlong-md.md)]或[!INCLUDE[addadapterservreflong](../../includes/addadapterservreflong-md.md)]，请参阅[获取 Visual Studio 中的 Siebel 操作的元数据](../../adapters-and-accelerators/adapter-siebel/get-metadata-for-siebel-operations-in-visual-studio.md)。  
