...
#### <a name="block-characters"></a>禁止字符  
 块字符参数是必须将接收或通过 MLLP 适配器发送的 HL7 消息的特殊字符。 这些字符会形成一个块格式如下： \<SB\>*DDD*\<EB\>\<CR\>，其中*DDD*代表消息数据， \<SB\>是开始块字符\<EB\>是结束块字符，并\<CR\>回车符。  
...
|使用此选项|执行的操作|  
|--------------|----------------|  
|按序送达|**MLLP 接收**:<br /><br /> 如果您设置**按序送达**属性设置为**FALSE** （默认值），实例文件将不会传递序列中。 将第一次传递推送到队列后较大的实例文件的较小的实例文件。 设置**按序送达**属性设置为**TRUE**、 来传送序列中的实例的文件。|  
...
|**TRUE**|0|连接永远不会中断。|  
|**TRUE**|<>0|导致错误条件:"接收超时值应为 TRUE 的持久连接的情况下零"。|  
