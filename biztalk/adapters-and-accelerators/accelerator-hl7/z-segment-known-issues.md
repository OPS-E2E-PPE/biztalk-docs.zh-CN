---
title: Z 段已知的问题 |Microsoft 文档
ms.custom: ''
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: ''
ms.suite: ''
ms.tgt_pltfrm: ''
ms.topic: article
helpviewer_keywords:
- segments, Z segments
- Z segments
- known issues, Z segments
- Z segments, known issues
ms.assetid: bf49d66b-f0c0-4e2b-9c08-bebb8f5bbc12
caps.latest.revision: 5
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 16251d607b357938439e95b949ef755936a13f38
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/20/2017
ms.locfileid: "22206557"
---
# <a name="z-segment-known-issues"></a>Z 段已知的问题
本节包含可以帮助您避免 Z 段错误的有用信息。  
  
## <a name="characters-not-recommended-to-use-for-naming-z-segments"></a>不建议用于命名 Z 段的字符  
 建议你不要命名 Z 段，因为它们是 HL7 段名称并可能会导致错误使用以下字符：  
  
-   FHS  
  
-   BHS  
  
-   BTS  
  
-   FTS  
  
## <a name="see-also"></a>另请参阅  
 [已知问题](../../adapters-and-accelerators/accelerator-hl7/known-issues1.md)