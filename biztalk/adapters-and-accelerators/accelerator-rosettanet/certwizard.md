...
|**Usage**|导入的私用证书的预期用途。 可以为 sign（表示用作签名证书）、decrypt（表示用作解密证书）、both（表示既用作签名证书也用作解密证书）或者 none（同样表示既用作签名证书也用作解密证书）。 **注意：** 如果你设置 **/Usage**交换机为 none，向导将 BizTalk 主机或 BizTalk 组上设置证书的指纹。|  
